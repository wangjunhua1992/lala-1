...
if($ta == 'consume_post') {
	...
	imessage(error(0, "核销订单成功"), ireferer(), 'ajax');
}
...
if($ta == 'refund') {
	...
	if($_W['ispost']) {
		...
		//$refund_fee = round($_GPC['refund_fee'], 2);
		...
		foreach($goods as $val) {
			if(!empty($refund_part[$val['id']])) {
				...
				} elseif($refund_item['total_num'] > $normal_num && $refund_item['total_num'] <= $val['goods_num']) {
				...
				$update_stat[$val['id']] = array(
					'goods_num' => $val['goods_num'] - $refund_item['total_num'],
					'goods_discount_num' => $val['goods_discount_num'] - $refund_discount_num,
					'goods_price' => $val['goods_price'] - round($refund_item_fee, 2),
					'goods_original_price' => $val['goods_price'] - round($refund_item_fee, 2),
				);
				//$refund_part[$val['id']]['refund_total_num'] = $refund_item['total_num'];
		...
		$update_order = array(
			'final_fee' => $order['final_fee'] - $calculate_refund_fee
		);
...
/*function order_goods_cancel_update($id, $refund_data, $reason) {
