...
if($ta == 'list') {
	...
	if($status == 3) {
		...
		if($config_takeout['order']['deliverynoassign_sort_type'] == 'desc'){
			$condition .= ' order by paytime desc';
		} else {
			$condition .= ' order by paytime asc';
		}
...
if($ta == 'detail') {
	...
	if(!empty($order['deliveryer_id']) && $order['deliveryer_id'] != $_deliveryer['id']) {
		imessage('该订单不是由您配送,无权查看订单详情', ireferer(), 'error');
	}
	if($order['transfer_delivery_status'] == 1) {
		imessage('您已经将该订单转给其他配送员,请等待其他配送员回复', ireferer(), 'info');
	}
...
if($ta == 'collect') {
	...
	imessage(error(0, '抢单成功'), ireferer(), 'ajax');
}
...
if($ta == 'success') {
	...
	imessage(error(0, '确认送达成功'), ireferer(), 'ajax');
}
...
if($ta == 'instore') {
	...
	imessage(error(0, '确认到店成功'), ireferer(), 'ajax');
}
...
if($ta == 'takegoods') {
	...
	imessage(error(0, '确认取货成功'), ireferer(), 'ajax');
}
...
if($ta == 'delivery_transfer') {
	...
	imessage(error(0, '转单成功'), ireferer(), 'ajax');
}
...
if($ta == 'cancel') {
	...
	imessage(error(0, '订单取消成功'), ireferer(), 'ajax');
}
