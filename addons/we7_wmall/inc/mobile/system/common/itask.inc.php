...
mload()->model('cron');
global $_W, $_GPC;
$task = cache_read('we7_wmall:task:businesstime');
if(!empty($task) && $task['expiretime'] > TIMESTAMP) {
	exit('process');
}
cache_write('we7_wmall:task:businesstime', array('expiretime' => TIMESTAMP + 120));
...
foreach($accounts as &$account) {
	...
	$key = "we7_wmall:{$_W['uniacid']}:task:lock:businesstime:180";
	if(!check_cache_status($key, 300)) {
		store_business_hours_init();
		set_cache($key, array());
	}
}
cache_delete('we7_wmall:task:businesstime');
exit('success');
