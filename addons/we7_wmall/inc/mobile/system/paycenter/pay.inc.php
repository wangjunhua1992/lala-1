...
$routers = array(
	...
	'mealRedpacket_plus' => array(
		...
		'url_pay' => imurl('system/paycenter/pay', array('id' => $order['id'], 'order_type' => 'mealRedpacket_plus', 'type' => 1), true),
		'url_detail' => imurl('mealRedpacket/plus/plusOrder', array(), true),
	),
);
...
if(empty($log)) {
	...
	//pdo_update('core_paylog', array('uniontid' => date('YmdHis') . random(14, 1)), array('plid' => $log['plid'], 'uniacid' => $_W['uniacid']));
	if($log['status'] == 1) {
		imessage('该订单已支付,请勿重复支付', '', 'error');
	}
}
