...
if($config_store['settle']['status'] != 1) {
	imessage('暂时不支持商户入驻', ireferer(), 'info');
}
$perm = check_max_store_perm();
if(empty($perm)) {
	imessage('门店入驻量已超过上限,请联系公众号管理员', ireferer(), 'info');
}
	...
	if($_W['isajax']) {
		...
		$data = array(
			...
			'qualification' => iserializer(array(
				'business' => array(
					'thumb' => trim($qualifications[0]),
				),
				'service' => array(
					'thumb' => trim($qualifications[1]),
				),
				'more1' => array(
					'thumb' => trim($qualifications[2]),
				)
			)),
		);
