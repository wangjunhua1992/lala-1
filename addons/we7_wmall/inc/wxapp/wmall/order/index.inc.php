...
if($ta == 'list') {
	...
	if(!empty($orders)) {
		...
		foreach($orders as &$da) {
			...
			if($da['status'] == 2 && $config_order['customer_cancel_status'] == 1 && $config_order['customer_cancel_timelimit'] > 0) {
				if(TIMESTAMP - $da['handletime'] < $config_order['customer_cancel_timelimit'] * 60) {
...
elseif($ta == 'detail') {
	...
	if($share['status'] == 1 && !$comment['is_share'] && $order['status'] == 5 && $order['endtime'] >= (time() - $share['share_grant_days_limit'] * 86400)) {
		...
		$_share = array(
			...
			'link' => imurl('/pages/ordergrant/share/detail', array('id' => $order['id']), 'true')
		);
	}
		...
		if(!empty($superRedpacket) && $superRedpacket['packet_dosage'] > 0) {
			...
			if(!empty($superRedpacket_share)) {
				...
				$_share = array(
					...
					'link' => "/package/pages/superredpacket/share?order_id={$id}"
				);
			}
		}
	}
	...
	if($order['status'] == 2 && $config_order['customer_cancel_status'] == 1 && $config_order['customer_cancel_timelimit'] > 0) {
		if(TIMESTAMP - $order['handletime'] < $config_order['customer_cancel_timelimit'] * 60) {
	...
	if(check_plugin_perm('svip') && $order['is_pay'] == 1 && $order['status'] < 5 && $order['order_type'] == 1 && !empty($update_address_config) && $update_address_config['status'] == 1 && $update_num < $update_address_config['address_update_num'] && $update_address_config['newaddress_distance'] > 0) {
	...
	$result = array(
		...
		'qrcode' => isurl('pages/order/detail', array('id' => $order['id'], 'sid' => $order['sid'], 'code' => $order['code']), true)
	);
	$_W['_nav'] = 1;
...
elseif($ta == 'new_detail') {
	...
	if($share['status'] == 1 && !$comment['is_share'] && $order['status'] == 5 && $order['endtime'] >= (time() - $share['share_grant_days_limit'] * 86400)) {
		...
		$_W['_share'] = array(
			...
			'desc' => $share['share']['desc'],
			'imgUrl' => tomedia($share['share']['imgUrl']),
			'link' => ivurl('/package/pages/ordergrant/detail', array('id' => $order['id']), 'true')
		);
	}
	...
	$result = array(
		...
		'can_delete' => $can_delete,
		'config_mall' => array(
			'call_deliveryer_need_select' => $_config_mall['call_deliveryer_need_select'],
...
elseif($ta == 'update_address') {
	...
	if($address_update_num >= $config['address_update_num']) {
	...
	if(!empty($distances)) {
		foreach($distances as $key => $value) {
			if($value['distance'] > $config['newaddress_distance'] * 1000) {
	...
	$addresses = array(
		...
		'dis_available' => array_values($disavailable)
	);
	$result = array(
		'order' => $order,
		'addresses' => $addresses,
...
elseif($ta == 'zhunshibao') {
	...
	if(empty($order[$config['start_time']])) {
		if($config['start_time'] == 'paytime') {
			$tip = '请在订单支付后查看';
		} elseif($config['start_time'] == 'handletime') {
			$tip = '请在商家接单后查看';
		} elseif($config['start_time'] == 'delivery_assign_time') {
			$tip = '请在配送员接单后查看';
		}
