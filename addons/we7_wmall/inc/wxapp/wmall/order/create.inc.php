...
if($ta == 'index') {
	...
	if($config_svip_status) {
		...
		$svip_redpacket = svip_store_useful_redpacket_get($store, $cart['price']+$cart['box_price'], array('is_buysvip' => $cart['is_buysvip']));
		if(!is_error($svip_redpacket)) {
			if($cart['is_buysvip'] == 1) {
	...
	$result = array(
		'store' => array_lang_translate($store),
		'address' => $address ? $address : array(),
		'cart' => $cart,
		'coupons' => coupon_available($sid, $cart['price']),
		'redPackets' => redPacket_available($cart['price'], explode('|', $store['cid']), array('scene' => 'waimai', 'sid' => $sid, 'order_type' => $condition['order_type'], 'agentid' => $store['agentid'])),
		'addresses' => member_fetchall_address_bystore($sid),
		'order' => $order,
		'message' => $message,
		'islegal' => 0,
		'mobile' => $_W['member']['mobile'],
		'config_takeout' => array(
			'audit_accept_address' => $_W['we7_wmall']['config']['takeout']['order']['audit_accept_address']
		)
	);
	...
	if(check_plugin_perm('huangou')) {
		...
		if($store['data']['huangou']['status'] == 1) {
			...
			$result['huangou'] = array(
				'activity' => $huangou['activity'],
				'goods' => $huangou_goods,
				'message' => $message_huangou,
				'cart_message' => $huangou_cart_message,
				'price_limit' => $huangou['price_limit'],
				'can_buy_huangou_goods' => ($order['total_fee_show'] >= $huangou['price_limit']) ? 1 : 0,
			);
		}
	}
...
elseif($ta == 'submit') {
	...
	$order = array(
		...
		'delivery_day' => $calculate['deliveryTimes']['predict_year_cn'] .'-'.$calculate['deliveryTimes']['predict_day_cn'],
		...
		'caigou_price' => $calculate['caigou_price'],
		...
		'invoice' => $invoice,
		'addtime' => TIMESTAMP,
		'person_num' => intval($store['data']['order_form']['person_num']) == 1 ? (intval($condition['person_num']) ? intval($condition['person_num']) : 1) : 0,
		'data' => array(
			'formId' => $condition['formId'],
			'extra_fee' => $calculate['extra_fee_detail'],
			'member_choice' => $calculate['member_choice_note'],
	...
	if($condition['buy_mealredpacket'] == 1) {
		$meal_redpacket_order = array(
			'uniacid' => $_W['uniacid'],
			'uid' => $_W['member']['uid'],
			'openid' => $_W['openid'],
			'sid' => $calculate['meal_redpackets']['superRedpacket_id'],
			'meal_id' => '',
			'order_sn' => date('YmdHis') . random(6, true),
			'final_fee' => floatval($calculate['meal_redpackets']['price']),
			'is_pay' => 0,
			'addtime' => TIMESTAMP,
			'type' => 'exchangeRedpacket',
			'data' => iserializer(array(
				'meal' => $calculate['meal_redpackets']
			))
		);
		pdo_insert('tiny_wmall_superredpacket_meal_order', $meal_redpacket_order);
		$meal_redpacket_order_id = pdo_insertid();
		$order['data']['meal_redpacket'] = array(
			'fee' => $meal_redpacket_order['final_fee'],
			'meal_order_id' => $meal_redpacket_order_id,
			'pre_mealredpacket_used_key' => !empty($calculate['meal_redpackets']['pre_mealredpacket_used_key']) ? $calculate['meal_redpackets']['pre_mealredpacket_used_key'] : 0,
		);
	...
	if(!empty($calculate['svip_meal'])) {
		$svip_order = array(
			...
			'openid' => $_W['openid'],
			'ordersn' => date('YmdHis') . random(6, true),
			'meal_id' => $calculate['svip_meal']['id'],
			'final_fee' => $calculate['svip_meal']['price'],
			'is_pay' => 0,
			'order_channel' => in_array($_W['ochannel'], array('wxapp', 'ttapp')) ? $_W['ochannel'] : 'wechat',
			'addtime' => TIMESTAMP,
			'data' => iserializer(array(
				'days' => $calculate['svip_meal']['days']
			))
		);
		pdo_insert('tiny_wmall_svip_meal_order', $svip_order);
		$svip_order_id = pdo_insertid();
		$order['data']['svip'] = array(
			'fee' => $svip_order['final_fee'],
			'svip_order_id' => $svip_order_id,
			'pre_svip_redpacket_id' => intval($calculate['pre_svip_redpacket_id'])
		);
...
elseif($ta == 'exchange') {
	...
	$result = array(
		...
		'svip_redpacket' => svip_store_useful_redpacket_get($store, $discount),
