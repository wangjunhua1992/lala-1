...
if($ta == 'post') {
	...
	$insert = array(
		...
		'delivery_service' => intval($_GPC['deliverScore']) ? intval($_GPC['deliverScore']) : 5,
		'note' => trim($_GPC['note']),
		'status' => $store['comment_status'],
		'data' => '',
		'addtime' => TIMESTAMP,
		'taste_score' => intval($_GPC['tasteScore']),
		'package_score' => intval($_GPC['packageScore']),
		'deliveryer_tag' => $delivery_tags
	);
	$thumbs = $_GPC['thumbs'];
	if(!is_array($thumbs)) {
