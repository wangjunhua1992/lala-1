...
if($ta == 'index') {
	...
	if($store['data']['tangshi']['pindan_status'] == 1) {
		...
		if(empty($table['cart_id']) && $table['status'] == 1) {
		...
		} elseif($table['cart_id'] > 0 && $table['status'] == 1) {
			...
			} elseif(empty($first_cart['pindan_id']) && $cart_id != $first_cart['id']) {
				$pindan_id = $first_cart['id'];
...
elseif($ta == 'cart') {
	...
	$ignore_bargain = $order_id > 0 ? true : false;
...
elseif($ta == 'detail') {
	...
	$goods['price'] = $goods['old_price'] = $goods['ts_price'];
...
elseif($ta == 'create') {
	...
	$order = array(
		'order_type' => 3,
		'price' => $cart['price'],
		'total_fee' => $cart['price'] + $serve_fee + $box_price,
		'serve_fee' => $serve_fee,
		'discount_fee' => $activityed['total'],
		'activityed' => $activityed,
		'note' => trim($condition['note']),
		'box_price' => $box_price,
		'box_price_tangshi' => $box_price_tangshi,
		'person_num' => $person_num,
		'invoiceId' => intval($condition['invoiceId']),
		'coupon' => coupon_available_check($sid, $coupon_id, $cart['price']),
		'redpacket' => redpacket_available_check($redpacket_id, $cart['price'], explode('|', $store['cid']), array('scene' => 'waimai', 'sid' => $sid, 'agentid' => $store['agentid']))
	);
	...
	$result = array(
		...
		'redPackets' => redPacket_available($cart['price'], explode('|', $store['cid']), array('scene' => 'waimai', 'sid' => $sid, 'agentid' => $store['agentid'])),
		'order' => $order,
		'message' => $message,
		'islegal' => 1,
	);
...
elseif($ta == 'submit') {
	...
	$order = array(
		...
		'final_fee' => $cart['price'] + $serve_fee + $box_price - $activityed['total'],
		'status' => 1,
		'is_comment' => 0,
		'invoice' => $invoice,
		'addtime' => TIMESTAMP,
		'data' => array(
			'cart' => $cart['original_data']
		),
		'note' => trim($condition['note']),
