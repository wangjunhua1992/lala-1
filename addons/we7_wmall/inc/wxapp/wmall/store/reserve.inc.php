...
if($ta == 'index') {
	...
	$params = array(
		...
		':sid' => $sid,
		':order_type' => 4,
		':stat_day' => date('Ymd', strtotime('-6 day')),
		':status' => 5,
	);
	...
	foreach($data as $da) {
		...
		$reserves[$da['table_cid']][] = array(
			'time' => $da['time'],
			'title' => empty($da['title']) ? $da['time'] : $da['title'],
			'timeout' => $timeout,
			'total_num' => intval($tables[$da['table_cid']]['total_num']),
		);
	}
...
elseif($ta == 'post') {
	...
	foreach($tables as &$table) {
		if($table['guest_num'] > 0) {
			$table['title'] = "{$table['title']}({$table['guest_num']}人桌)";
		}
	}
	...
	$result = array(
		...
		'order' => $order,
		'cart' => $cart,
		'columns' => array('1人', '2人', '3人', '4人', '5人', '6人', '7人', '8人', '9人', '10人', '11人', '12人', '13人', '14人', '15人', '16人', '17人', '18人', '19人', '20人'),
		'islegal' => true
	);
	imessage(error(0, $result), '', 'ajax');
}
...
elseif($ta == 'submit') {
	...
	$order = array(
		...
		'table_cid' => $cid,
		'table_id' => intval($condition['table_id']),
