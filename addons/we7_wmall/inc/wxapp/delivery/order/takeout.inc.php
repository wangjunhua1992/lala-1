...
if($ta == 'list') {
	...
	if(!empty($orders)) {
		...
		foreach($orders as &$da) {
			...
			if($da['status'] == 4 && $config_takeout['order']['delivery_timeout_limit'] > 0) {
	...
	$result = array(
		...
		'config' => array(
			'auto_refresh' => $config_takeout['order']['auto_refresh'],
		),
		'userAgreement' => get_config_text('deliveryer:agreement')
	);
...
elseif($ta == 'detail') {
	...
	if($order['status'] == 4 && $config_takeout['order']['delivery_timeout_limit'] > 0) {
