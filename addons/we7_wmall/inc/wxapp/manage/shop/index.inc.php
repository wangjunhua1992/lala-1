...
if($ta == 'index') {
	...
	$notice = store_notice_stat($_W['manager']['id'], $store['agentid']);
...
elseif($ta == 'info') {
	...
	} elseif($type == 'haodian_category') {
		$filter = array('haodian_cid', 'haodian_child_id');
	} elseif($type == 'haodian_tags') {
...
elseif($ta == 'status') {
	...
	if(in_array($type, array('is_in_business', 'auto_handel_order', 'auto_notice_deliveryer'))) {
		if($type == 'auto_handel_order' && $_W['we7_wmall']['config']['takeout']['order']['can_set_auto_handel_order'] === 0) {
			imessage(error(0, '平台已关闭此项设置的权限，如需设置请联系平台管理员'), '', 'ajax');
		}
		$update = array(
			$type => $value
		);
...
elseif($ta == 'pill') {
	if($_W['ispost']) {
		...
		$update = array(
			...
			'auto_print_order' => intval($store['auto_handel_order']) == 2 ? 0 : intval($store['auto_print_order']),
...
elseif($ta == 'slog') {
	...
	if(!empty($type) && !empty($message)) {
		slog($type, "{$title}:{$_W['manager']['title']}", '', $message);
	}
