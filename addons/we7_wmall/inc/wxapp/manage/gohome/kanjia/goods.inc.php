...
if($ta == 'post') {
	...
	if($_W['ispost']) {
		...
		$data = array(
			...
			'agentid' => $_W['agentid'],
			'sid' => $sid,
			'cateid' => intval($value['cateid']),
			'oldprice' => floatval($value['oldprice']),
			'price' => floatval($value['price']),
			'submitmoneylimit' => intval($value['submitmoneylimit']),
			'starttime' => strtotime($value['starttime_cn']),
			'endtime' => strtotime($value['endtime_cn']),
			'helplimit' => intval($value['helplimit']),
			'dayhelplimit' => intval($value['dayhelplimit']),
			'joinlimit' => intval($value['joinlimit']),
			'usestatus' => intval($value['usestatus']),
			'falsejoinnum' => intval($value['falsejoinnum']),
			'falselooknum' => intval($value['falselooknum']),
			...
			'detail' => htmlspecialchars_decode($value['detail']),
			'addtime' => TIMESTAMP,
		);
		$data['thumbs'] = array();
