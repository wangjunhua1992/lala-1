...
if($ta == 'paytype' || $ta == 'message') {
	...
	if(empty($paylog)) {
		imessage(error(-1, '订单交心记录不存在'), '', 'ajax');
	}
...
if($ta == 'message') {
	...
	if(empty($peerpay)) {
		$peerpay = array(
			...
			'data' => $paylog['data'],
			'peerpay_message' => $config_peerpay['help_words'][0]
		);
	...
	$result = array(
		'member_avatar' => tomedia($member['avatar']),
		'peerpay' => $peerpay,
		'help_words' => !empty($config_peerpay['help_words']) ? shuffle($config_peerpay['help_words']) : '',
		'page_title' => '编辑代付留言',
	);
...
if($ta == 'paylist') {
	...
	$result = array(
		...
		'differ' => $differ,
		'percent' => $percent,
		'page_title' => empty($member['nickname']) ? '我的代付' : "{$member['nickname']}的代付",
		'is_same_person' => $_W['member']['uid'] == $member['uid'] ? 1 : 0,
	);
	$title = '亲爱哒，帮我付一下呗~';
	...
	$_W['_share'] = array(
		...
		'link' => ivurl('pages/public/peerpay/paylist', array('id' => $peerpay['id']), true)
	);
	imessage(error(0, $result), '', 'ajax');
}
