...
if($op == 'settle') {
	...
	if($_W['ispost']) {
		$settle = array(
			...
			'qualification_verify_status' => intval($_GPC['qualification_verify_status']),
		...
		imessage(error(0, '商户入驻设置成功'), ireferer(), 'ajax');
	}
...
elseif($op == 'serve_fee') {
	...
	if($_W['ispost']) {
		...
		if($form_type == 'serve_fee_setting') {
			...
			if($serve_fee['fee_takeout']['type'] == 2) {
			...
			} elseif($serve_fee['fee_takeout']['type'] == 3) {
				$serve_fee['fee_takeout']['level1'] = array(
				...
				$serve_fee['fee_takeout']['level2'] = array(
			...
			} elseif($serve_fee['fee_takeout']['type'] == 4) {
				$serve_fee['fee_takeout']['single_percent'] = array(
			...
			if($serve_fee['fee_selfDelivery']['type'] == 2) {
			...
			} elseif($serve_fee['fee_selfDelivery']['type'] == 3) {
				$serve_fee['fee_selfDelivery']['level1'] = array(
				...
				$serve_fee['fee_selfDelivery']['level2'] = array(
			...
			} elseif($serve_fee['fee_selfDelivery']['type'] == 4) {
				$serve_fee['fee_selfDelivery']['single_percent'] = array(
			...
			if($serve_fee['fee_instore']['type'] == 2) {
			...
			} elseif($serve_fee['fee_instore']['type'] == 3) {
				$serve_fee['fee_instore']['level1'] = array(
				...
				$serve_fee['fee_instore']['level2'] = array(
			...
			} elseif($serve_fee['fee_instore']['type'] == 4) {
				$serve_fee['fee_instore']['single_percent'] = array(
			...
			if($serve_fee['fee_paybill']['type'] == 2) {
			...
			} elseif($serve_fee['fee_paybill']['type'] == 3) {
				$serve_fee['fee_paybill']['level1'] = array(
				...
				$serve_fee['fee_paybill']['level2'] = array(
		...
		} elseif($form_type == 'getcash_fee_setting') {
		...
		$data = array(
			'fee_takeout' => iserializer($serve_fee['fee_takeout']),
			'fee_selfDelivery' => iserializer($serve_fee['fee_selfDelivery']),
			'fee_instore' => iserializer($serve_fee['fee_instore']),
			'fee_paybill' => iserializer($serve_fee['fee_paybill']),
			...
			'fee_period' => $serve_fee['fee_period']
		);
		if($form_type == 'serve_fee_setting') {
			$update = array(
				'fee_takeout' => $data['fee_takeout'],
				'fee_selfDelivery' => $data['fee_selfDelivery'],
				'fee_instore' => $data['fee_instore'],
				'fee_paybill' => $data['fee_paybill']
			);
		} elseif($form_type == 'getcash_fee_setting') {
			$update = array(
				'fee_limit' => $data['fee_limit'],
...
elseif($op == 'delivery') {
	...
	if($_W['ispost']) {
		if ($_GPC['form_type'] == 'another') {
		...
		} elseif($_GPC['form_type'] == 'reserve') {
			$reserve_data = array(
				'rest_can_order' => intval($_GPC['rest_can_order']),
				'reserve' => array(
					'reserve_time_limit' => intval($_GPC['reserve']['reserve_time_limit']),
					'notice_clerk_before_delivery' => intval($_GPC['reserve']['notice_clerk_before_delivery'])
				)
			);
			set_system_config('store.reserve', $reserve_data);
			$update_reserve = array(
				'rest_can_order' => $reserve_data['rest_can_order']
			);
			...
			imessage(error(0, '预订单设置成功'), ireferer(), 'ajax');
		} elseif($_GPC['form_type'] == 'pindan') {
			$pindan_data = array(
			...
			imessage(error(0, '配送费设置成功'), ireferer(), 'ajax');
		}
	}
	...
	$item = array(
		'isChange' => 1,
		'delivery_areas' => get_config_text('store_delivery_areas'),
...
elseif($op == 'extra') {
	...
	if($_W['ispost']) {
		...
		if($form_type == 'another_setting') {
			...
			$update = array(
				'remind_time_start' => $extra['remind_time_start'],
			...
			set_system_config('store.extra', $extra);
		} elseif($form_type == 'kefu') {
			$extra['kefu_status'] = intval($_GPC['kefu_status']);
			$update = array(
				'kefu_status' => $extra['kefu_status']
			);
			set_system_config('store.extra', $extra);
		} else {
			...
			imessage(error(0, '模板配置成功'), ireferer(), 'ajax');
		}
		$extra_sync = intval($_GPC['extra_sync']);
		if($extra_sync == 1) {
			if(!empty($update)) {
		...
		if($form_type == 'another_setting') {
			foreach($store_ids as $storeid) {
				...
				store_stat_init('delivery_time', $storeid);
			}
		} elseif($form_type == 'extra_setting') {
			foreach($store_ids as $storeid) {
...
elseif($op == 'activity') {
	...
	if($_W['ispost']) {
		...
		foreach($_GPC['activity']['status'] as $type => $val) {
			$data[$type] = array(
				'status' => intval($val),
				'cancel_status' => intval($_GPC['activity']['cancel_status'][$type]),
			);
		}
		set_system_config('store.activity.perm', $data);
		imessage(error(0, '设置商户活动成功'), ireferer(), 'ajax');
	}
...
elseif($op == 'recommend') {
	...
	if($_W['ispost']) {
		$recommend = array(
			'image' => trim($_GPC['selective_image'])
		);
		set_system_config('selective', $recommend);
		imessage(error(0, '设置为您优选图片成功'), ireferer(), 'ajax');
	}
