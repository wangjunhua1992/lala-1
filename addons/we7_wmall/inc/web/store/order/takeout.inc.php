...
if($ta == 'list'){
	...
	//订单统计
	$condition = ' where uniacid = :uniacid and sid = :sid and status = 5 and is_pay = 1 and order_type < 3 and stat_day = :stat_day';
	...
	if(!empty($orders)) {
		...
		foreach($orders as &$da) {
			...
			if($da['status'] == 2 && $config_takeout['customer_cancel_status'] == 1 && $config_takeout['customer_cancel_timelimit'] > 0) {
...
if($ta == 'push_dada') {
	...
	if ($_W['ispost']) {
		$status = order_push_dada($id, $push);
		if(is_error($status)) {
			imessage($status, ireferer(), 'ajax');
		}
		$fee = floatval($status['fee']);
		if(!$push) {
			$result = array(
				'tips' => "达达配送需要配送费{$fee}{$_W['Lang']['dollarSignCn']}，确定推送到达达吗？",
				'id' => $id
			);
		} else {
			$result = '订单成功推送到达达配送';
