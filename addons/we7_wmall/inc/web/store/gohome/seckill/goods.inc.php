...
if ($ta == 'post') {
	...
	if ($_W['ispost']) {
		...
		$data = array(
			...
			'buy_note' => htmlspecialchars_decode($_GPC['buy_note']),
			'status' => intval($_GPC['status']),
			'starttime' => intval($starttime),
			'endtime' => intval($endtime),
			'use_limit_day' => intval($_GPC['use_limit_day']),
		);
		$data['thumbs'] = array();
		...
		if(!empty($_GPC['ids'])) {
			foreach ($_GPC['ids'] as $k => $v) {
				$data = array(
					...
					'oldprice' => floatval($_GPC['old_price'][$k]),
					'total' => intval($_GPC['total'][$k]),
					//'displayorder' => intval($_GPC['displayorder'][$k]),
					'use_limit_day' => intval($_GPC['use_limit_day'][$k]),
				);
