...
if($ta == 'post') {
	...
	if($_W['ispost']) {
		$title = !empty($_GPC['title']) ? trim($_GPC['title']) : imessage(error(-1, '活动主题不能为空'), ireferer(), 'ajax');
		$goods = array();
		if(!empty($_GPC['goods_id'])) {
			foreach($_GPC['goods_id'] as $key => $goods_id) {
				...
				if(empty($temp) || ($temp['bargain_id'] > 0 && $temp['bargain_id'] != $id) || !empty($temp['attrs'])) {
					continue;
				}
				$row = array(
					'goods_id' => $goods_id,
		...
		if(empty($goods)) {
			imessage(error(-1, '请选择参与活动的商品'), ireferer(), 'ajax');
		}
		...
		$activity = array(
			...
			'data' => iserializer(array(
				'price_limit' => floatval($_GPC['price_limit'])
			))
		);
	...
	if($id > 0) {
		...
		if(empty($bargain)) {
			imessage('换购活动不存在或已删除', ireferer(), 'error');
		}
		$activity = activity_get($sid, 'huangou');
