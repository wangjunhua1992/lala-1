...
if($ta == 'create') {
	...
	$everyone_num = intval($_GPC['everyone_num']) > 0 ? intval($_GPC['everyone_num']) : 1;
	if($_W['ispost']) {
		...
		$coupon = array(
			...
			'data' => iserializer(array(
				'total_fee' => $group['uids_total'] * intval($_GPC['discount']) * $everyone_num,
				'grant_object' => array(
					'uids' => $group['uids'],
					'total' => $group['uids_total'] * $everyone_num,
					'unissued_uid' => $group['uids'],
					'grant_success' => 0,
					'everyone_num' => $everyone_num
				),
				'coupon' => array(
