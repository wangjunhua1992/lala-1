...
if($ta == 'post') {
	...
	if($id) {
		...
		if(empty($item)) {
			...
			$config_sfexpress = $item['data']['sfexpress'];
		...
		if($_W['role'] == 'merchanter') {
			imessage('您没有该添加门店的权限', ireferer(), 'error');
		}
	...
	if(check_plugin_perm('area') && $config_mall['address_type'] == 1) {
		mload()->model('plugin');
		pload()->model('area');
		$areas_cache = area_plateform_area_all(true);
	...
	if($_W['ispost']) {
		$data = array(
			...
			'auto_print_order' => intval($_GPC['auto_handel_order']) == 2 ? 0 : intval($_GPC['auto_print_order']),
		...
		$cn = array(
			'box_price' => !empty($box_price)? $box_price : '餐盒费',
			'pack_fee' => !empty($pack_fee)? $pack_fee : '包装费'
		);
		store_set_data($id, 'cn', $cn);
		$order_form = array(
			'person_num' => intval($_GPC['order_form']['person_num']),
		);
		...
		if($address_type == 1) {
			...
			if(!empty($areas_gpc) && !empty($areas_gpc['id'])) {
				foreach($areas_gpc['ids'] as $key => $childid) {
					...
					if(!empty($status)) {
						$delivery_area1[$childid] = array(
							'id' => $childid,
							'parentid' => intval($areas_gpc['parentid'][$key]),
							'price' => floatval($areas_gpc['price'][$key])
						);
					}
				}
			}
		...
		$uupaotui_data = array(
			'status' => intval($_GPC['uupaotui']['status']),
			'appid' => trim($_GPC['uupaotui']['appid']) ? trim($_GPC['uupaotui']['appid']) : $config_uupaotui['appid'],
			'appkey' => trim($_GPC['uupaotui']['appkey']) ? trim($_GPC['uupaotui']['appkey']) : $config_uupaotui['appkey'],
			'openid' => trim($_GPC['uupaotui']['openid']) ? trim($_GPC['uupaotui']['openid']) : $config_uupaotui['openid']
		);
		store_set_data($id, 'uupaotui', $uupaotui_data);

		$shansong_data = array(
			'status' => intval($_GPC['shansong']['status']),
			'mobile' => trim($_GPC['shansong']['mobile']),
			'md5' => trim($_GPC['shansong']['md5']),
			'token' => trim($_GPC['shansong']['shansongtoken']),
			'merchantid' => trim($_GPC['shansong']['merchantid']),
			'partnerNO' => trim($_GPC['shansong']['partnerNO'])
		);
		store_set_data($id, 'shansong', $shansong_data);

		$dianwoda_data = array(
			'status' => intval($_GPC['dianwoda']['status']),
			'appkey' => trim($_GPC['dianwoda']['appkey']),
			'appsecret' => trim($_GPC['dianwoda']['appsecret']),
			'accesstoken' => trim($_GPC['dianwoda']['accesstoken']),
			'merchantid' => trim($_GPC['dianwoda']['merchantid']),
		);
		store_set_data($id, 'dianwoda', $dianwoda_data);

		$sfexpress_data = array(
			'status' => intval($_GPC['sfexpress']['status']),
			'dev_id' => trim($_GPC['sfexpress']['dev_id']),
			'dev_key' => trim($_GPC['sfexpress']['dev_key']),
			'shop_id' => trim($_GPC['sfexpress']['shop_id'])
		);
		store_set_data($id, 'sfexpress', $sfexpress_data);
	...
	$can_set_auto_handel_order = 1;
	if($_W['role'] == 'merchanter' && $_W['we7_wmall']['config']['takeout']['order']['can_set_auto_handel_order'] === 0) {
		$can_set_auto_handel_order = 0;
	}
	$pay = get_available_payment();
	if(empty($pay)) {
		imessage('公众号没有设置支付方式,请先设置支付方式', ireferer(), 'info');
	}
