...
$permissions = array(
	...
	'agenter' => array(
		'controller' => array('common', 'order', 'paycenter', 'merchant', 'store', 'dashboard', 'deliveryer', 'clerk', 'memmber', 'finance', 'config', 'service', 'plugin', 'statcenter', 'oauth', 'plugin', 'errander', 'bargain', 'agent', 'diypage', 'zhunshibao', 'superRedpacket', 'perm'),
	),
	'agent_operator' => array(
		'controller' => array('common', 'order', 'paycenter', 'merchant', 'store', 'dashboard', 'deliveryer', 'clerk', 'memmber', 'finance', 'config', 'service', 'plugin', 'statcenter', 'oauth', 'plugin', 'errander', 'bargain', 'agent', 'diypage', 'zhunshibao', 'superRedpacket', 'perm'),
	),
);
...
if($_W['role'] == 'manager') {
	...
	if(!in_array($_W['_controller'], $permissions['agent_operator']['controller']) && !defined('IN_GOHOME')) {
		$permission = false;
	}
}
...
if($_W['role'] == 'operator' || $_W['role'] == 'agent_operator') {
	...
	if(strexists($perm, "merchant.account") || strexists($perm, "deliveryer.plateform") || strexists($perm, "agent.agent")) {
		$perm = "{$_W['_controller']}.{$_W['_action']}.{$_W['_op']}";
	}
...
if(defined('IN_MERCHANT')) {
	...
	if($_W['_op'] != 'login') {
		...
		if($_W['is_agent'] && ($_W['role'] == 'agenter' || $_W['role'] == 'agent_operator') && $store['agentid'] != $_W['agentid']) {
			imessage("您好,{$_W['we7_wmall']['agent']['realname']},该门店不属于您的管辖,您无权操作", ireferer(), 'error');
		}
		...
		if(($_W['role'] == 'agenter' || $_W['role'] == 'agent_operator') && $_W['store']['agentid'] != $_W['agentid']) {
			imessage('该门店不属于您的管辖,您无权操作！', ireferer(), 'error');
		}
	}
	...
	if($_W['_action'] != 'login') {
		...
		if(empty($agent)) {
			imessage('代理不存在或已删除！', ireferer(), 'error');
		}
...
if($_W['role'] == 'manager') {
	...
	$_W['user'] = array(
		'uid' => $_W['agent']['id'],
		'utype' => 'agenter',
		'realname' => $_W['agent']['realname'],
		'nickname' => $_W['agent']['nickname'],
		'avatar' => $_W['agent']['avatar'],
	);
