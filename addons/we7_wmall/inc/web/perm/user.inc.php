	...
	if($_W['ispost']) {
		...
		if(empty($id)) {
			if(!preg_match(REGULAR_USERNAME, $member['username'])) {
				imessage(error(-1, '必须输入用户名，格式为 3-15 位字符，可以包括汉字、字母（不区分大小写）、数字、下划线和句点。'), ireferer(), 'ajax');
			}
			if(user_check(array('username' => $member['username']))) {
				imessage(error(-1, '非常抱歉，此用户名已经被注册，你需要更换注册名称！'), ireferer(), 'ajax');
			}
			$member['password'] = $_GPC['password'];
			if(istrlen($member['password']) < 8) {
				imessage(error(-1, '必须输入密码，且密码长度不得低于8位。'), ireferer(), 'ajax');
			}
			$member['starttime'] = TIMESTAMP;
			$uid = user_register($member, 'admin');
			if(is_error($uid)) {
				imessage(error(-1, "密码不合法。{$uid['message']}"), ireferer(), 'ajax');
			}
			$insert['uid'] = $uid;
