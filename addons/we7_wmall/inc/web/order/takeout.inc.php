...
if($op == 'detail') {
	...
	$order['spread'] = order_fetch_spread($order);
	$order['storebd'] = order_fetch_storebd($order);
...
if($op == 'export0') {
	...
	$order_fields = array(
		...
		'store_discount_fee' => array(
			'field' => 'store_discount_fee',
			'title' => '商户承担金额',
			'width' => '15',
		),
		'agent_discount_fee' => array(
			'field' => 'agent_discount_fee',
			'title' => '代理商承担金额',
			'width' => '15',
		),
		'plateform_discount_fee' => array(
			'field' => 'plateform_discount_fee',
			'title' => '平台承担金额',
			'width' => '15',
		),
		'final_fee' => array(
	...
	if(!empty($list)) {
		...
		foreach($goods_temp as $row) {
			if(!empty($row['goods_number'])) {
				$row['goods_title'] = "{$row['goods_title']}-{$row['goods_number']}";
			}
			$goods[$row['oid']][] = "{$row['goods_title']} X {$row['goods_num']}";
		}
...
if($op == 'export') {
	...
	if($page > 0){
		for($pindex = 1; $pindex <= $page; $pindex++) {
			$list = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall_order') . $condition . ' ORDER BY id DESC LIMIT '.($pindex - 1) * 1000 . ', 1000', $params);
			if(!empty($list)) {
				$oids = array();
				foreach($list as $li) {
					$oids[] = $li['id'];
				...
				for($i = 0, $length = count($list); $i < $length; $i++) {
					...
					foreach($all_fields as $key => $li) {
						...
						$objPHPExcel->getActiveSheet(0)->setCellValue($key . (($pindex - 1) * 1000 + $i + 2), $row[$field]);
					}
				}
			}
			unset($list, $goods_temp, $goods);
		}
	}
	$objPHPExcel->getActiveSheet()->setTitle('订单数据');
	$objPHPExcel->setActiveSheetIndex(0);
...
if($op == 'push_uupaotui') {
	...
	if ($_W['ispost']) {
		...
		if(!$push) {
			$result = array(
				'tips' => "UU跑腿需要配送费{$status['need_paymoney']}{$_W['Lang']['dollarSignCn']}，确定推送到UU跑腿吗？",
				'id' => $id
			);
		} else {
			$result = '订单推送到UU跑腿成功';
		}
		imessage(error(0, $result), ireferer(), 'ajax');
	}
}
...
if($op == 'push_shansong') {
	...
	if ($_W['ispost']) {
		...
		if(!$push) {
			$result = array(
				'tips' => "闪送配送需要配送费{$status['amount']}{$_W['Lang']['dollarSignCn']}，确定推送闪送吗？",
				'id' => $id
			);
		} else {
			$result = '订单推送到闪送成功';
...
if($op == 'push_dianwoda') {
	...
	if ($_W['ispost']) {
		...
		if(!$push) {
			$result = array(
				'tips' => "点我达配送需要配送费{$fee}{$_W['Lang']['dollarSignCn']}，确定推送点我达吗？",
				'id' => $id
			);
		} else {
			$result = '订单成功推送到点我达';
...
if($op == 'push_sfexpress') {
	$id = intval($_GPC['id']);
	$push = intval($_GPC['push']) ? true : false;
	if ($_W['ispost']) {
		$status = order_push_sfexpress($id, $push);
		if(is_error($status)) {
			imessage($status, ireferer(), 'ajax');
		}
		$fee = floatval($status['total_price'] / 100);
		if(!$push) {
			$result = array(
				'tips' => "顺丰同城配送需要配送费{$fee}{$_W['Lang']['dollarSignCn']}，确定推送顺丰同城吗？",
				'id' => $id
			);
		} else {
			$result = '订单成功推送到顺丰同城';
