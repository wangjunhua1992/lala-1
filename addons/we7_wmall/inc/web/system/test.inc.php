...
if($op == 'copyall') {
	/*pdo_query('delete from ' . tablename('tiny_wmall_store') . ' where uniacid = :uniacid and is_copy = 1', array(':uniacid' => $uniacid_new));
	$tables = array(
		'tiny_wmall_store_account', 'tiny_wmall_goods_category', 'tiny_wmall_goods',
		'tiny_wmall_goods_options', 'tiny_wmall_tables_category', 'tiny_wmall_tables',
		'tiny_wmall_assign_queue', 'tiny_wmall_reserve'
	);
	foreach($tables as $table) {
		pdo_delete($table, array('uniacid' => $uniacid_new, 'is_copy' => 1));
	}
	echo '删除成功';
	die();*/
	...
	foreach($stores as $store) {
		...
		unset($table_categorys, $reserves);
...
if($op == 'gohome_copy') {
	...
	unset($kanjia_goods, $kanjia_category_old2new);
	echo '砍价商品ok';
	...
	unset($goods, $pintuan_category_old2new);
	echo '拼团商品ok';
	...
	unset($goods, $seckill_category_old2new, $store_old2new);
	echo '抢购商品ok';
	...
	unset($tongcheng_categorys, $old_new_cid);
	echo '同城分类ok';
	...
	unset($haodian_categorys, $old_new_cid);
	echo '好店ok';
