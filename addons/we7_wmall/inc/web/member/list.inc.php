...
if($op == 'cancel') {
	...
	imessage(error(0, '套餐取消成功'), ireferer(), 'ajax');
}
...
if($op == 'changes') {
	...
	if(empty($member)) {
		imessage(error(-1, '会员不存在或已经删除'), ireferer(), 'ajax');
	}
	if($_W['ispost']) {
		$type = trim($_GPC['type']);
		...
		imessage(error(0,"{$credit_text}变动成功"), ireferer(), 'ajax');
	}
...
if($op == 'setmeal') {
	...
	if($_W['ispost']) {
		if(empty($id)){
			imessage(error(-1, '请选择需要修改的会员'), ireferer(), 'ajax');
		}
		...
		imessage(error(0, '套餐修改成功'), ireferer(), 'ajax');
	}
...
if($op == 'del') {
	...
	if(empty($id) || empty($uid)) {
		imessage(error(0, '顾客不存在'), ireferer(), 'ajax');
	}
	...
	$tables = array(
		...
		'tiny_wmall_activity_redpacket_record',
		'tiny_wmall_address',
		'tiny_wmall_assign_board',
		'tiny_wmall_creditshop_order',
		'tiny_wmall_delivery_cards_order',
		'tiny_wmall_freelunch_partaker',
		'tiny_wmall_member_footmark',
		'tiny_wmall_member_invoice',
		'tiny_wmall_member_recharge',
		'tiny_wmall_notice_read_log',
		'tiny_wmall_order_cart',
		'tiny_wmall_perm_user',
		'tiny_wmall_report',
		'tiny_wmall_store_favorite',
		'tiny_wmall_store_members',
		'tiny_wmall_superredpacket_grant'
	);
...
if($op == 'info') {
	...
	if(empty($member)) {
		imessage(error(-1, '顾客不存在或已被删除'), ireferer(), 'error');
	}
	if ($_W['ispost']){
		$member = array(
...
if($op == 'svip_status') {
	...
	if($_W['ispost']) {
		...
		if($status == '1') {
			...
			if(TIMESTAMP > $svip_endtime) {
				imessage(error(-1, '会员开始时间不能大于结束时间'), ireferer(), 'ajax');
			}
			if($member['svip_status'] == '1') {
				$data = array(
					'svip_endtime' => $svip_endtime
				);
			} elseif($member['svip_status'] == '2') {
				$data = array(
					'svip_status' => '1',
					'svip_endtime' => $svip_endtime
				);
			...
			imessage(error(0, '续费超级会员成功'), ireferer(), 'ajax');
		} elseif($status == '2') {
			if($member['svip_status'] == '0' || $member['svip_status'] == '2') {
				imessage(error(0, '撤销超级会员成功'), ireferer(), 'ajax');
			} else {
				$data = array(
					'svip_status' => 2,
