...
class Wxapp extends TyAccount {
	...
	public function sendTplNotice_old($touser, $template_id, $postdata, $url, $form_id, $color = '#FF683F') {
	...
	public function sendTplNotice($touser, $template_id, $postdata, $url) {
		...
		$data = json_encode($data);
		$post_url = "https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token={$token}";
		$response = ihttp_request($post_url, $data);
		if(is_error($response)) {
			return error(-1, "访问公众平台接口失败, 错误: {$response['message']}");
		}
	...
	public function errorCode($code, $errmsg = '未知错误') {
		$errors = array(
			...
			'40099' => '该code已被核销。',
			'61005' => '缺少接入平台关键数据，等待微信开放平台推送数据，请十分钟后再试或是检查“授权事件接收URL”是否写错（index.php?c=account&amp;a=auth&amp;do=ticket地址中的&amp;符号容心被替换成&amp;amp;）',
			'61023' => '请重新授权接入该公众号',
		);
		$code = strval($code);
		if($errors[$code]) {
