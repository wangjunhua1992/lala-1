 ...
 *  2. 链接是当前页面的锚点，并且该锚点对应的源素存在且符合路由约定，那么则把该源素做动画切入
 ...
 *  把所有页面的 js 都放到同一个脚本里，js 里面的事件绑定使用委托而不是直接的绑定在源素上（因为动态加载的页面源素还不存在），然后所有页面都引用相同的 js 脚本。非事件类可以通过监控 pageInit 事件，根据里面的 pageId 来做对应区别处理。
 *
 * 如果有需要
 *
 * 对外暴露的方法
 ...
 *  - 块（section），一个文档内有指定块标识的源素
 *
 * 路由实现约定
 *  - 每个文档的需要展示的内容必需位于指定的标识（routerConfig.sectionGroupClass）的源素里面，默认是: div.page-group （注意,如果改变这个需要同时改变 less 中的命名）
 ...
 *
 * 为了解决 ajax 载入页面导致重复 ID 以及重复 popup 等功能，上面约定了使用路由功能的所有可展示内容都必需位于指定源素内。从而可以在进行文档间切换时可以进行两个文档的整体移动，切换完毕后再把前一个文档的内容从页面之间移除。
 *
...
+function($) {
	 ...
	 * - 把当前文档内容缓存起来
	 * - 查找默认展示的块内容，查找顺序如下
	 *      1. id 是 url 中的 fragment 的源素
	 *      2. 有当前块 class 标识的第一个源素
	 *      3. 第一个块
	 * - 初始页面 state 处理
	 *
	 * @private
	 ...
	 * @param $from 目前显示的文档
	 * @param $to 待切换显示的新文档
	 * @param $visibleSection 新文档中展示的 section 源素
	 ...
	 * @param {String} sectionId 新状态中显示的 section 源素的 id
	 * @private
	 */
 ...
 * @property {*|HTMLElement} $content $doc 里的 routerConfig.innerViewClass 源素
 */
