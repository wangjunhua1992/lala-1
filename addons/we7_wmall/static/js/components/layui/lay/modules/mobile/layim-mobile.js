...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto'], function(exports){
  ...
  //接受订单消息
  LAYIM.prototype.getOrderMsg = function(order) {
    return getOrderMsg(order), this;
  };
  ...
  var elemChatMain = ['<li class="layim-chat-li{{ d.mine ? " layim-chat-mine" : "" }}">'
    ...
    ,'<div class="layim-chat-text">' +
			'{{# if(d.msg_type == 3){ }}' +
			'<div>门店: {{ d.content.title }}</div>' +
			'<div>商品: {{ d.content.goods_title }}</div>' +
			'<div>价格: ¥{{ d.content.final_fee }}</div>' +
			'<div>状态: {{ d.content.status_cn }}</div>' +
			'<div>单号: {{ d.content.ordersn }}</div>' +
			'{{# } else { }}' +
  ...
  var sendMessage = function(){
	...
	var sendOrderMsg = function(){
		...
		var order = JSON.parse(decodeURIComponent(thatChat.elem.find('#send-order-msg').data('order')));
		console.log(order)
  ...
  var messageNew = {}, getMessage = function(data){
	...
	var getOrderMsg = function(order){
		if(!layimChat || !order) return;

		var cont = layimChat.find('.layim-chat')
			,ul = cont.find('.layim-chat-main ul');
		var content = 	'<div class="layim-chat-order">' +
							'<div class="layim-chat-order-header">' +
								'<img class="order-store-logo" src="' + order.logo +'" alt="">' +
								'<span class="order-store-title">' + order.title + '</span>' +
							'</div>'+
							'<div class="layim-chat-order-body">' +
								'<div class="info-item">' +
									'<div class="order-goods-title">' + order.goods_title + '</div>' +
									'<div claass="order-final-fee">¥' + order.final_fee + '</div>' +
								'</div>' +
								'<div class="info-item">' +
									'<div class="order-addtime">' + order.addtime_cn +'</div>' +
									'<div class="order-status">' + order.status_cn + '</div>' +
								'</div>' +
							'</div>' +
							'<div class="layim-chat-order-footer" id="send-order-msg" layim-event="sendOrderMsg" data-order="' + encodeURIComponent(JSON.stringify(order)) +'">发送链接>></div>' +
						'</div>';
		ul.append(content);
	};
