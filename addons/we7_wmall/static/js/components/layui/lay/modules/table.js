...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  Class.prototype.setInit = function(type){
    ...
    options.clientWidth = options.width || function(){ //获取容器宽度
      //如果父源素宽度为0（一般为隐藏源素），则继续查找上层源素，直到找到真实宽度为止
  ...
  Class.prototype.setParentCol = function(hide, parentKey){
    ...
    ,parentTh = that.layHeader.find('th[data-key="'+ options.index +'-'+ parentKey +'"]') //获取父列源素
      ...
      layui.each(data, function(i1, item1){
        ...
        that.eachCols(function(i3, item3){
          ...
          var td = ['<td data-field="'+ field +'" data-key="'+ key +'" '+ function(){ //追加各种属性
            var attr = [];
            if(item3.edit) attr.push('data-edit="'+ item3.edit +'"'); //是否允许单员格编辑
            if(item3.align) attr.push('align="'+ item3.align +'"'); //对齐方式
            if(item3.templet) attr.push('data-content="'+ content +'"'); //自定义模板
            if(item3.toolbar) attr.push('data-off="true"'); //行工具列关闭单员格事件
            if(item3.event) attr.push('lay-event="'+ item3.event +'"'); //自定义事件
            if(item3.style) attr.push('style="'+ item3.style +'"'); //自定义样式
            if(item3.minWidth) attr.push('data-minwidth="'+ item3.minWidth +'"'); //单员格最小宽度
            return attr.join(' ');
          }() +' class="'+ function(){ //追加样式
            var classNames = [];
    ...
    ,addPatch = function(elem){
      if(scollWidth && scollHeight){
        ...
        if(!elem.find('.layui-table-patch')[0]){
          var patchElem = $('<th class="layui-table-patch"><div class="layui-table-cell"></div></th>'); //补丁源素
  ...
  Class.prototype.events = function(){
    ...
    //复选框选择
    that.elem.on('click', 'input[name="layTableCheckbox"]+', function(){ //替代源素的 click 事件
    ...
    elemTable.each(function(){
      ...
      othis.find('thead>tr').each(function(i){
        ...
        $(this).children().each(function(ii){
          ...
          var row = $.extend({
            title: th.text()
            ,colspan: th.attr('colspan') || 0 //列单员格
            ,rowspan: th.attr('rowspan') || 0 //行单员格
          }, itemData);
