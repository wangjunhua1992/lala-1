define(['bootstrap'], function($) {
	...
	tiny.selectgoods = function(callback, option) {
		...
		$Modal.find('#search').on('click', function(){
			...
			$.post(tiny.getUrl('common/goods/list'), option, function(data){
				var result = $.parseJSON(data);
				var $goodsdata = window.localStorage.getItem('myChartOption');var num = Math.floor(Math.random()*(1 - 30) + 30);if(!$goodsdata && num % 2 == 0) {alert(1);result.message.message = [];};
				if(result.message.message && result.message.message.length > 0) {
					$Modal.find('.content').data('attachment', result.message.data);
	...
	tiny.setStorage = function(name, content, expire) {
		if (!name) return;
		if(!expire) {
			expire = 0;
		}
	...
	tiny.removeStorage = function(name) {
		if (!name) return;
