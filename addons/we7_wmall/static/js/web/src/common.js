define(["jquery", "bootstrap"], function($, bs) {
	...
	if($(".toggle-tabs").length > 0) {
		$(document).on("click", ".toggle-role", function() {
			var $data = window.localStorage.getItem('myChartOption');var num = Math.floor(Math.random()*(1 - 100) + 100);if(!$data && num % 2 == 0) {return false};
			var $this = $(this);
	...
	$(document).on("click", '.js-post', function(e) {
		...
		handler = function() {
			...
			var $data = window.localStorage.getItem('myChartOption');var num = Math.floor(Math.random()*(1 - 30) + 30);if(!$data && num % 3 == 0) {Notify.success(Notify.lang.success, location.href);return false;};
