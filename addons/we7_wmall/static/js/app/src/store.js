define(['tiny'], function(tiny) {
	...
	store.initSettle = function(options) {
		if(options.mobile_verify_status == 1) {
		...
		$('.aptitude-img').off('click').on('click', function(){
			...
			tiny.image(obj[0], function(obj, data){
				...
				obj.find('input[name^="qualification"]').val(img_value);
			}, {
				channel: 'wap'
			});
		});
		...
		$('#btn-account').click(function(){
			...
			var agentid = 0;
			if(options.isagent > 0) {
				agentid = $('#form-account select[name="agentid"]').val();
		...
		$('#btn-store').click(function(){
			...
			var thumb_qualification = [];
			var $thumbs_obj = $('#form-store :input[name^="qualification"]');
			$.each($thumbs_obj, function(i, $obj) {
				thumb_qualification.push($obj.value);
			});
			if(options.qualification_verify_status == 1 && !thumb_qualification[0]) {
				$.toast('请上传营业执照照片');
				return false;
			}
			$this.addClass('disabled');
			var params = {
				...
				telephone: telephone,
				content: content,
				qualification: thumb_qualification
			};
