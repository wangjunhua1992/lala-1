...
if($op == 'post') {
	...
	if($_W['ispost']) {
		...
		if(!empty($exchanges) && $_GPC['data']['params']['exchangeStatus'] == 1) {
			foreach($exchanges as $exchange) {
				if($exchange['store_id'] > 0) {
					$item = array(
						'uniacid' => $_W['uniacid'],
						'redpacketid' => $id,
						'sid' => $exchange['store_id'],
						'discount' => $exchange['discount'],
						'condition' => $exchange['condition'],
						'grant_days_effect' => $exchange['grant_days_effect'],
						'use_days_limit' => $exchange['use_days_limit'],
					);
	...
	if(!empty($id)) {
		...
		if(!empty($mealRedpacket_yes)) {
			imessage('已有套餐红包活动, 如需重新添加套餐红包活动，请先撤销其他活动', ireferer(), 'info');
		}
	}
