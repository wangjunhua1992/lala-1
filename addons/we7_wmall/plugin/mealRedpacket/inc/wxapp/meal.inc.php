...
if($op == 'submit') {
	...
	$order = array(
		...
		'uid' => $_W['member']['uid'],
		'openid' => $_W['openid'],
		'sid' => $mealRedpacket_id,
		'type' => 'exchangeRedpacket',
		'order_sn' => date('YmdHis') . random(6, true),
		'final_fee' => floatval($activity['data']['params']['price']),
		'is_pay' => 0,
		'addtime' => TIMESTAMP,
...
if($op == 'do_exchange') {
	...
	$update = array(
		...
		'starttime' => TIMESTAMP + $exchange['grant_days_effect'] * 86400,
		'endtime' => TIMESTAMP + $exchange['grant_days_effect'] * 86400 + $exchange['use_days_limit'] * 86400,
		'times_limit' => ''
	);
