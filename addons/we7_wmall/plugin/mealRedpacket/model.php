...
function mealRedpacket_exchanges_getall($id) {
	...
	$psize = intval($_GPC['psize']) > 0 ? intval($_GPC['psize']) : 15;
...
function mealRedpacket_build_virtual() {
	...
	if(!empty($result)) {
		mload()->model('redPacket');
		$meal_redpackets = array(
			'price' => $result['data']['params']['price'],
			'title' => $result['data']['params']['title'],
			'placeholder' => $result['data']['params']['placeholder'],
		);
