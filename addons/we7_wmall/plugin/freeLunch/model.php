...
function freelunch_record_init() {
	...
	if(empty($record) || empty($record['partaker_dosage'])) {
		...
		$insert = array(
			'uniacid' => $_W['uniacid'],
			'freelunch_id' => $freelunch['id'],
			'serial_sn' => $serial_sn ? $serial_sn : 1,
			'type' => 'common',
			'partaker_fee' => $freelunch['pre_partaker_fee'],
			'partaker_total' => $freelunch['pre_partaker_num'],
			'partaker_dosage' => $freelunch['pre_partaker_num'],
			'reward_fee' => $freelunch['pre_reward_fee'],
			'startime' => TIMESTAMP,
		);
	...
	if((empty($record) || empty($record['partaker_dosage'])) && $freelunch['plus_status'] == 1) {
		...
		$insert = array(
			...
			'serial_sn' => $serial_sn ? $serial_sn : 1,
			'type' => 'plus',
			'partaker_fee' => $freelunch['plus_pre_partaker_fee'],
			'partaker_total' => $freelunch['plus_partaker_num'],
			'partaker_dosage' => $freelunch['plus_partaker_num'],
			'reward_fee' => $freelunch['pre_plus_reward_fee'],
			'startime' => TIMESTAMP,
		);
		pdo_insert('tiny_wmall_freelunch_record', $insert);
	}
...
function freelunch_partaker_partake_status($uid, $id, $type) {
	global $_W;
	$freelunch = freelunch_get();
	if($freelunch['max_partake_times'] > 0) {
	...
	if($member_partaker['nums'] >= $pre_max_partake_times && $pre_max_partake_times > 0) {
	...
	if($type == 'pay') {
		...
		if($freelunch['partake_grant_type'] == 1) {
			...
			$params = array(
				...
				'type' => 'common',
				'uid' => $order['uid'],
				'discount' => $order['final_fee'],
				'condition' => 0,
				'days_limit' => $freelunch['redpacket_days_limit'],
			);
			redPacket_grant($params);
		} elseif($freelunch['partake_grant_type'] == 2) {
			mload()->model('member');
			$log = array(
				$record['uid'],
				"霸王餐返现金{$order['final_fee']}{$_W['Lang']['dollarSignCn']}"
			);
...
function freelunch_reward($record_id) {
	...
	if($freelunch['reward_grant_type'] == 1) {
		...
		$params = array(
			...
			'uid' => $partaker['uid'],
			'condition' => 0,
			'discount' => $record['reward_fee'],
			'days_limit' => $freelunch['redpacket_days_limit'],
		);
		redPacket_grant($params);
	} elseif($freelunch['reward_grant_type'] == 2) {
		mload()->model('member');
		$log = array(
			$partaker['uid'],
			"霸王餐获奖返现金{$record['reward_fee']}{$_W['Lang']['dollarSignCn']}"
		);
