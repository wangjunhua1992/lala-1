...
if($op == 'index') {
	...
	$result = array(
		'freelunch' => $freelunch,
		'record' => $record,
		'partake_status' => $partake_status,
		'member_partaker' => $member_partaker,
		'luckiers' => $luckiers
	);
...
if($op == 'partake') {
	...
	if($partake_status['errno'] == -1) {
		imessage($partake_status['message'], ireferer(), 'info');
	}
	$insert = array(
		'uniacid' => $_W['uniacid'],
		'freelunch_id' => $freelunch['id'],
		'record_id' => $record['id'],
		'serial_sn' => $record['serial_sn'],
		'uid' => $_W['member']['uid'],
		'number' => '',
		'addtime' => TIMESTAMP,
		'final_fee' => $record['partaker_fee'],
...
if($op == 'detail') {
	...
	$result = array(
		'freelunch' => $freelunch,
		'record' => $record,
		'mine_partaker' => $mine_partaker,
		'partake_status' => $partake_status,
		'winner_partaker' => $winner_partaker,
		'member' => $member,
		'uid' => $_W['member']['uid']
	);
	imessage(error(0, $result), '', 'ajax');
