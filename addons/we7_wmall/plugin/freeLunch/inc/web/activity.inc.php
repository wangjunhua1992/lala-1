...
if($op == 'config') {
	...
	if($_W['ispost']) {
		...
		$data = array(
			...
			'max_partake_times' => intval($_GPC['max_partake_times']),
			'partake_grant_type' => intval($_GPC['partake_grant_type']),
			'reward_grant_type' => intval($_GPC['reward_grant_type']),
			'redpacket_days_limit' => intval($_GPC['redpacket_days_limit']),
			'pre_partaker_num' => intval($_GPC['pre_partaker_num']),
			'pre_partaker_fee' => floatval($_GPC['pre_partaker_fee']),
			'pre_reward_fee' => floatval($_GPC['pre_reward_fee']),
			'pre_max_partake_times' => intval($_GPC['pre_max_partake_times']),
			'plus_status' => intval($_GPC['plus_status']),
			'plus_thumb' => trim($_GPC['plus_thumb']),
			'plus_reward_num' => intval($_GPC['plus_reward_num']),
			'plus_partaker_num' => intval($_GPC['plus_partaker_num']),
			'plus_pre_partaker_fee' => floatval($_GPC['plus_pre_partaker_fee']),
			'pre_plus_reward_fee' => floatval($_GPC['pre_plus_reward_fee']),
			'plus_pre_max_partake_times' => intval($_GPC['plus_pre_max_partake_times']),
			'share' => iserializer($share),
			'agreement' => htmlspecialchars_decode($_GPC['agreement']),
			'status' => intval($_GPC['status']),
			'credit_pay_status' => intval($_GPC['credit_pay_status']),
			'starttime' => $starttime,
			'endtime' => $endtime,
			'addtime' => TIMESTAMP
		);
		...
		imessage(error(0, '设置活动成功'), ireferer(), 'ajax');
	}
...
if($op == 'period_del') {
	...
	imessage(error(0, '删除活动期数成功'), ireferer(), 'ajax');
}
