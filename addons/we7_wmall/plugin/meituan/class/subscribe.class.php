	...
	public function parse() {
		...
		if($type == 'order') {
			...
			$order = array(
				...
				'acid' => $_W['acid'],
				'sid' => $store['id'],
				'uid' => 0, //$message['userId']
				'order_type' => $address[0] == '到店自取' ? 2 : 1,
				'meituanOrderId' => $message['orderId'],
				'ordersn' => $message['orderId'],
				'address' => $address[0],
				'addtime' => $message['utime'],
				'paytime' => $message['utime'],
				'note' => $message['caution'],
				'invoice' => $message['invoiceTitle'],
				'mobile' => $message['recipientPhone'],
				'total_fee' => $message['originalPrice'],
				'final_fee' => $message['total'],
				'store_final_fee' => 0,
				'meituan_store_final_fee' => round($message['poiReceiveDetail']['wmPoiReceiveCent'] / 100, 2),
				'username' => substr($message['recipientName'], 0, strpos($message['recipientName'], '(')),
				'sex' => substr($message['recipientName'], strpos($message['recipientName'], '(') + 1, 6),
				'location_x' => $message['latitude'],
				'location_y' => $message['longitude'],
				'order_plateform' => 'meituan',
				'order_channel' => 'meituan',
				'delivery_type' => 0,
				'delivery_fee' => $message['shippingFee'],
				'delivery_time' => !empty($message['deliveryTime']) ? date('Y-m-d H:i', $message['deliveryTime']) : '立即配送',
				'is_pay' => 1,
				'pay_type' => $message['payType'] == 2 ? 'meituan' : 'delivery',
				'serial_sn' => $message['daySeq'],
				'plateform_serve_rate' => 0,
				'plateform_serve_fee' => 0,
				'box_price' => 0,
				'plateform_discount_fee' => $message['poiReceiveDetail']['actOrderChargeByMt']['moneyCent'],
				'store_discount_fee' => $message['poiReceiveDetail']['actOrderChargeByPoi']['moneyCent'],
				'discount_fee' => 0,
				'plateform_delivery_fee' => 0,
				'person_num' => $message['dinnersNumber'],
				'status' => $this->orderStatusTransform($message['status']),
				'stat_year' => date('Y', $message['utime']),
				'stat_month' => date('Ym', $message['utime']),
				'stat_day' => date('Ymd', $message['utime']),
			);
			...
			if(!empty($config_meituan)) {
				if($config_meituan_delivery['delivery_mode'] == 1) {
				...
				} elseif($config_meituan_delivery['delivery_mode'] == 2)  {
					...
					if($config_meituan_delivery['delivery_fee_mode'] == 1) {
						...
						if($distance > $config_meituan_delivery['delivery_price']['start_km']) {
			...
			$plateform_serve = array(
				...
				'fee_rate' => 0,
				'fee' => 0,
				'note' => "每单固定0{$_W['Lang']['dollarSignCn']}",
				'meituan' => $meituan
			);
			if(!empty($account['fee_meituan'])) {
				if($account['fee_meituan']['fee_type'] == 1) {
					...
					$plateform_serve = array(
						'fee_type' => 1,
						'fee_rate' => $account['fee_meituan']['fee_rate'],
						'fee' => $platform_serve_fee,
						'note' => "(美团平台最终入账 {$_W['Lang']['dollarSign']}{$order['meituan_store_final_fee']}) x {$account['fee_meituan']['fee_rate']}% {$text}",
						'meituan' => $meituan
					);
				} else {
			...
			if(!empty($order['invoice'])) {
				$invoice = array(
					'invoiceTitle' => $message['invoiceTitle'], //发票抬头
					'taxpayerId' => $message['taxpayerId'], //发票税号
				);
			}
			$order['data'] = iserializer(array(
				'invoice' => $invoice,
			));
			...
			foreach($message['detail'] as $item) {
					...
					$stat = array(
						...
						'sid' => $order['sid'],
						'goods_title' => $title,
						'goods_unit_price' => $item['price'],
						'goods_num' => $item['quantity'],
						'goods_price' => round($item['quantity'] * $item['price'], 2),
						'goods_original_price' => round($item['quantity'] * $item['price'], 2),
						'addtime' => $order['addtime'],
						'order_plateform' => 'meituan',
						'stat_year' => date('Y', $order['addtime']),
						'stat_month' => date('Ym', $order['addtime']),
						'stat_day' => date('Ymd', $order['addtime']),
						'stat_week' => date('YW', $order['addtime'])
					);
			...
			if(!empty($message['extras'])) {
				foreach($message['extras'] as $orderActivity) {
					$discount = array(
						...
						'type' => 'discount',
						'icon' => 'discount_b.png',
						'name' => $orderActivity['remark'],
						'note' => "-{$_W['Lang']['dollarSign']}{$orderActivity['reduce_fee']}",
						'fee' => $orderActivity['reduce_fee'],
						'store_discount_fee' => $orderActivity['poi_charge'],
						'plateform_discount_fee' => $orderActivity['mt_charge'],
					);
					pdo_insert('tiny_wmall_order_discount', $discount);
				}
			}
			/*
			 * 这里进行自动接单.自动打印.自动通知配送员。
			 * */
		...
		elseif($type == 'orderShippingStatus') {
			...
			} elseif($message['shippingStatus'] == 40) {
				order_status_update($order['id'], 'end', array('role' => 'meituan'));
			}
		}
		echo '{"data":"ok"}';
