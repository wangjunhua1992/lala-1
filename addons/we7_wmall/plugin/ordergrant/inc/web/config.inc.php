...
if($_W['ispost']) {
	...
	if(!empty($_GPC['continuous_days'])) {
		foreach($_GPC['continuous_days'] as $key => $row) {
			...
			$continuous[$key] = array(
				'days' => $row,
				'grant' => $_GPC['continuous_grant'][$key]
			);
		}
	}
	$all = array();
	...
	if(!empty($_GPC['special_date'])) {
		foreach($_GPC['special_date'] as $key => $row) {
			...
			$special[$key] = array(
				'date' => $row,
				'title' => $_GPC['special_title'][$key],
				'color' => $_GPC['special_color'][$key],
				'grant' => $_GPC['special_grant'][$key]
			);
		}
	}
	$ordergrant_share = array(
	...
	$ordergrant = array(
		...
		'first_order_grant' => floatval($_GPC['first_order_grant']),
		'days_order_grant' => floatval($_GPC['days_order_grant']),
		'continuous' => $continuous,
		'special' => $special,
		'all' => $all,
		'agreement' => htmlspecialchars_decode($_GPC['agreement']),
		'ordergrant_share' => $ordergrant_share
	);
