...
class DaDa{
	...
	public function __construct() {
		...
		$api_urls = array(
			'open' => 'http://newopen.imdada.cn',
			'sandbox' => 'http://newopen.qa.imdada.cn'
		);
	...
	public function buildBodyParams($id) {
		...
		$params = array(
			'shop_no' => $dada['shopno'],                      //文档提供
			'origin_id' => $order['ordersn'],           //第三方订单ID
			'city_code' => $dada['citycode'],
			'cargo_price' => $order['final_fee'],
			'is_prepay' => 0,                           //是否需要垫付
			'expected_fetch_time' => TIMESTAMP + 60*10,   //期望取货时间10min
			'receiver_name' => $order['username'],
			'receiver_address' => $order['address'],    //'马驹桥金桥花园北门',
			'receiver_phone' => $order['mobile'],        //必填
			'receiver_lat' => $order['location_x'],
			'receiver_lng' => $order['location_y'],
			'info' => $order['note'],
			'callback' => imurl('dada/api', array(), true),
		);
		return $params;
	}
	...
	public function queryDeliverFee($id) {
		...
		unset($params['expected_fetch_time']);
	...
	public function orderAccept($id) {
		$order = order_fetch($id);
		$params = array(
			'order_id' => $order['ordersn']       //第三方订单编号
		);
