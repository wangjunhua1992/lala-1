...
elseif($op == 'post') {
	...
	if($_W['ispost']) {
		$update = array(
			'content' => trim($publish['content']),
			'nickname' => trim($publish['nickname']),
			'mobile' => trim($publish['mobile']),
		);
		...
		if($calculate['final_fee'] > 0) {
			$update_trade = array(
				...
				'tid' => $infor_id,
				'uid' => $_W['member']['uid'],
				'type' => $calculate['is_stick'],
				'price' => $calculate['price'],
				'stick_price' => $calculate['stick_price'],
