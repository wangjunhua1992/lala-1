...
function tongcheng_get_informations($filter = array()) {
	...
	if(!empty($informations)) {
		$categorys_all = tongcheng_get_categorys(array('type' => 'all'), array('id', 'title', 'thumb'));
		...
		foreach($informations as &$val) {
			...
			$cid = ($filter['show_child_category_cn'] && $val['childid']) ? $val['childid'] : $val['parentid'];
			$val['category'] = $categorys_all[$cid];
...
function tongcheng_information_publish_calculate($categoryOrId, $condition) {
	...
	if(!empty($category)) {
		...
		if($condition['information_id'] > 0) {//编辑信息不受费
			$price = 0;
		}
		...
		$result = array(
			'price' => $price,
			'stick_price' => $stick_fee,
			'is_stick' => $stick_fee > 0 ? 1 : 0,
			'days' => $day,
			'stick_is_available' => $stick_is_available
		);
...
function tongcheng_information_update($orderOrId, $type, $extra = array()) {
	...
	if($type == 'pay') {
		...
		if($order['type'] == 0 || $order['type'] == 1) {
			...
			} elseif($information['edit_status'] == 1) {
...
function tongcheng_flow_update($type = '') {
	...
	if(in_array($type, array('falselooknum', 'falsefabunum', 'falselikenum'))) {
		$add_num = 1;
		if($type == 'falselooknum') {
			if($config['minup'] > 0 && $config['maxup'] >= $config['minup']) {
	...
	return array(
		'falselooknum' => intval($config['falselooknum']),
		'falsefabunum' => intval($config['falsefabunum']),
		'falselikenum' => intval($config['falselikenum']),
	);
}
...
function tongcheng_tiezi_notice($tieziOrId, $type, $extra = array()) {
	...
	if($channel == 'wap') {
		...
		$params = array(
			'first' => $first[$type],
			'keyword1' => $extra['nickname'],
			'keyword2' => date('Y-m-d H:i', $extra['addtime']),
			'keyword3' => '审核通过',
		);
		$extra['note'] = array(
			"评论内容:" => $extra['content'],
		...
		if($config_wxapp_basic['tpl_consumer_url'] == 'wxapp') {
			$miniprogram = array(
				...
				'pagepath'=> "gohome/pages/tongcheng/detail?id={$tiezi['id']}",
			);
		}
	...
	} elseif($channel == 'wxapp') {
		$params = array(
			'keyword1' => $extra['content'],
			'keyword2' => $extra['nickname'],
			'keyword3' => date('Y-m-d H:i', $extra['addtime']),
			'keyword4' => $tiezi['content'],
		);
		$send = sys_wechat_tpl_format($params);
