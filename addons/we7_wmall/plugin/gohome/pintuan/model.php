	...
	if(!empty($goods)) {
		...
		foreach($goods as &$da) {
			...
			if($da['total'] != -1) {
				...
				$da['sailed_percent'] = ($da['falesailed_total'] > $orgin_total) ? round($da['sailed'] / $orgin_total * 100, 1) : round(($da['falesailed_total']) / $orgin_total * 100, 1);
			}
		}
	}
	return $goods;
...
function pintuan_get_activity($id) {
	...
	if(!empty($activity)) {
		...
		if($activity['total'] != -1) {
			...
			$activity['sailed_percent'] = ($activity['sailed'] + $activity['falesailed'] > $orgin_total) ? round($activity['sailed'] / $orgin_total * 100, 1) : round(($activity['sailed'] + $activity['falesailed']) / $orgin_total * 100, 1);
		}
...
function pintuan_order_calculate($goodsOrId, $condition, $extra = array()) {
	...
	} elseif($pintuan_goods['usetype'] == 2) {
		if(!empty($condition)) {
	...
	$goods_num = 1;
	if($pintuan_goods['buylimit'] > 1 || !$pintuan_goods['buylimit']) {
		$goods_num = intval($condition['goods_num']) ? intval($condition['goods_num']) : 1;
		if($pintuan_goods['buylimit'] > 1 && $goods_num > $pintuan_goods['buylimit']) {
	...
	$order = array(
		'goods_num' => $goods_num,
		'address' => $address,
		'goods_price' => round($goods_num * $pintuan_goods['price'], 2),
		'discount' => ($pintuan_goods['oldprice'] - $pintuan_goods['price']) > 0 ? $goods_num * ($pintuan_goods['oldprice'] - $pintuan_goods['price']) : 0,
		'discount_real' => 0,
		'remark' => trim($condition['remark']),
		'store' => $store,
		'username' => empty($condition['username']) ? $_W['member']['realname'] : trim($condition['username']),
		'mobile' => empty($condition['mobile']) ? $_W['member']['mobile'] : trim($condition['mobile']),
	);
...
function pintuan_is_available($goodsOrId, $team_id = 0) {
	...
	if(TIMESTAMP <= $goods['starttime'] || TIMESTAMP >= $goods['endtime']) {
	...
	if($team_id > 0) {
		...
		if($teams_length >= $goods['peoplenum']) {
			return error(-1000, '该团购人数已达上限，请重新下单');
		}
		if($goods['grouptime'] > 0 && TIMESTAMP - $teams[0]['paytime'] > $goods['grouptime'] * 3600) {
...
function pintuan_order_update($orderOrId, $type, $extra = array()) {
	...
	if($type == 'pay') {
		...
		if(empty($order['is_team'])) {
			$update['status'] = 3;
		} elseif ($order['is_team'] == 1 && $order['id'] == $order['team_id']) {
		...
		$teams[] = $order;
		if($extra['team_cancel'] == 1 && $order['team_id'] > 0) {
