...
function gohome_update_activity_flow($activity_type, $goods_id, $type) {
	...
	$routers = array(
		...
		'tongcheng' => array(
			'table' => 'tiny_wmall_tongcheng_information'
		),
	);
...
function gohome_order_begin_refund($orderOrid) {
	...
	if($order['pay_type'] == 'credit') {
		if($order['uid'] > 0) {
			$log = array(
				$order['uid'],
				"{$order['order_type_cn']}订单退款, 订单号:{$order['id']}, 退款金额:{$order['final_fee']}{$_W['Lang']['dollarSignCn']}",
				'we7_wmall'
			);
			mload()->model('member');
	...
	if($order_channel == 'wechat') {
		if($type == 'pay') {
			...
			$remark = array(
				...
				"完成时间: " . date('Y-m-d H:i', $order['applytime']),
			);
			$order['status_cn'] = '已核销';
		...
		if($config_wxapp_basic['tpl_consumer_url'] == 'wxapp') {
			$miniprogram = array(
				...
				'pagepath'=> "gohome/pages/order/detail?id={$order['id']}",
			);
		}
		$params_send = array(
			'title' => $title,
...
function gohome_complain_notice($idOrComplain) {
	...
	if(!empty($complain)) {
		$complain_types = array(
			...
			'privacy' => array(
				'type' => 'privacy',
				'text' => '网页在收集个人隐私信息（如：钓鱼链接）',
			),
			'induce' => array(
				'type' => 'induce',
				'text' => '网页包含诱导分享/关注性质的内容',
			),
			'rumor' => array(
				'type' => 'rumor',
				'text' => '网页可能包含谣言信息',
		...
		$title = '有顾客投诉了，请处理';
		$remark = array(
			"投诉　人: {$_W['member']['nickname']}",
			"顾客编号: {$_W['member']['uid']}",
			"投诉原因: {$complain_types[$complain['type']]['text']}",
			"投诉时间: " . date('Y-m-d H:i', $complain['addtime']),
		);
...
function gohome_order_clerk_notice($id, $type, $note = '') {
	...
	//生活圈订单通知，仅支持微信公众号和APP通知，不支持小程序（小程序没有实现生活圈的相关功能）
	$store = $order['store'];
	...
	if($type == 'pay') {
		...
		$remark = array(
			"门店名称： {$store['title']}",
			"订单备注：{$order['buyremark']}",
			"商品信息： {$order['goods']['name']} x {$order['num']}",
			"下单时间： {$order['addtime_cn']}",
		...
		$remark = array(
			...
			"订单类型: {$order['order_type_cn']}",
			"商品信息： {$order['goods']['name']} x {$order['num']}",
			"核销时间: " . date('Y-m-d H:i', $order['applytime']),
		);
	} elseif($type == 'team_success') {
		...
		$remark = array(
			"门店名称: {$store['title']}",
			"拼团商品: {$order['goods']['name']}",
			"参团人数: {$order['takepart_num']}",
		);
	} elseif($type == 'cancel') {
	...
	if($channel_notice == 'wechat') {
		...
		//$url = isurl('pages/gohome/order/detail', array('id' => $order['id']), true);
		$url = '';
		$remark = implode("\n", $remark);
		$miniprogram = '';
		$params_send = array(
...
function gohome_get_menu($type = '') {
	...
	$menus = array(
		'gohome' => array(
			...
			'data' => array (
				'M0123456789101' => array (
					'link' => '/gohome/pages/home/index',
					'icon' => 'icon-home',
					'text' => '首页',
				),
				'M0123456789102' => array (
					'link' => '/gohome/pages/member/favorite',
					'icon' => 'icon-likefill',
					'text' => '收藏',
				),
				'M0123456789103' => array (
					'link' => '/gohome/pages/order/index',
					'icon' => 'icon-order',
					'text' => '订单',
				),
		...
		'pintuan' => array(
			...
			'data' => array (
				'M0123456789101' => array (
					'link' => '/gohome/pages/pintuan/index',
					'icon' => 'icon-home',
					'text' => '首页',
				),
				'M0123456789103' => array (
		...
		'kanjia' => array(
			...
			'data' => array (
				'M0123456789101' => array (
					'link' => '/gohome/pages/kanjia/index',
					'icon' => 'icon-home',
					'text' => '首页',
				),
				'M0123456789103' => array (
					'link' => '/gohome/pages/kanjia/record',
					'icon' => 'icon-friend',
					'text' => '砍价',
				),
				'M0123456789104' => array (
		...
		'seckill' => array(
			...
			'data' => array (
				'M0123456789101' => array (
					'link' => '/gohome/pages/seckill/index',
					'icon' => 'icon-home',
					'text' => '首页',
				),
				'M0123456789102' => array (
		...
		'tongcheng' => array(
			...
			'data' => array (
				...
				'M0123456789102' => array (
					'link' => '/gohome/pages/tongcheng/publish/index',
					'icon' => 'icon-edit',
					'text' => '发布',
				),
				'M0123456789103' => array (
					'link' => '/gohome/pages/tongcheng/publish/list',
					'icon' => 'icon-message',
					'text' => '我的发布',
				),
				'M0123456789104' => array (
		...
		'haodian' => array(
			...
			'data' => array (
				...
				'M0123456789103' => array (
					'link' => '/gohome/pages/haodian/favorite',
					'icon' => 'icon-likefill',
					'text' => '收藏',
				),
				'M0123456789104' => array (
...
function gohome_order_print($id) {
	...
	foreach($prints as $li) {
		if(!empty($li['print_no'])) {
			$content = array(
				'title' => "<CB>{$_W['we7_wmall']['config']['mall']['title']}</CB>",
				'orderfrom' => "<CB>{$order['order_type_cn']}订单</CB>",
