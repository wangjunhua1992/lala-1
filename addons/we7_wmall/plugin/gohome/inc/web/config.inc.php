...
if($op == 'activity') {
	...
	if($_W['ispost']) {
		if($_GPC['form_type'] == 'basic_setting') {
			$data = array(
				'black_tip' => trim($_GPC['black_tip']),
				'status' => array_map('intval', $_GPC['status']),
				'danmu_status' => array_map('intval', $_GPC['danmu_status'])
			);
			set_plugin_config('gohome.basic', $data);
		} else {
			$config['credit1'] = array(
		...
		imessage(error(0, '保存成功'), ireferer(), 'ajax');
	}
...
elseif($op == 'serve_fee') {
	...
	if($_W['ispost']) {
		...
		imessage(error(0, '费率设置成功'), ireferer(), 'ajax');
	}
...
elseif($op == 'share') {
	...
	if($_W['ispost']) {
		...
		imessage(error(0, '分享设置设置成功'), ireferer(), 'ajax');
	}
