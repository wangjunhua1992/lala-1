...
if(empty($perm)) {
	imessage(error(-1,'门店入驻量已超过上限,请联系公众号管理员'), ireferer(), 'ajax');
}
...
if($op == 'account') {
	...
	if($_W['ispost']) {
		...
		$data = array(
			...
			'openid' => $_W['openid_wechat'],
			'openid_wxapp' => $_W['openid'],
	...
	//onload成功返回判断短信验证
	$result = array(
		'mobile_verify_status' => $config_store['settle']['mobile_verify_status'],
...
elseif($op == 'store') {
	if(empty($clerk)) {
		imessage(error(-1000,'请申请商户入驻'), '', 'ajax');//跳转‘settle/account’
	}
	...
	if($_W['ispost']) {
		...
		$qualifications = in_array($_W['ochannel'], array('wxapp', 'ttapp')) ? $GPC_store['qualification'] : $_GPC['qualification'];
		if($config_store['settle']['qualification_verify_status'] == 1 && empty($qualifications[0]['filename'])) {
			imessage(error(-1, '请上传营业执照照片'), '', 'ajax');
		}
		$data = array(
			'uniacid' => $_W['uniacid'],
			'agentid' => $clerk['agentid'] ? $clerk['agentid'] : $_W['agentid'],
			'title' => $title,
			'logo' => trim($GPC_store['logo']),
			'address' => trim($GPC_store['address']),
			'telephone' => trim($GPC_store['telephone']),
			'content' => trim($GPC_store['content']),
			...
			'haodian_cid' => intval($GPC_store['haodian_cid']),
			'haodian_child_id' => intval($GPC_store['haodian_child_id']),
			'haodian_status' => 5,//待审核
			'is_haodian' => 1,
			'is_waimai' => 0
		);
		...
		$GPC_thumbs =  in_array($_W['ochannel'], array('wxapp', 'ttapp')) ? $GPC_store['thumbs'] : $_GPC['thumbs'];
		if(!empty($GPC_thumbs)) {
		...
		$meal = in_array($_W['ochannel'], array('wxapp', 'ttapp')) ? $GPC_store['meal'] : $_GPC['meal'];
		if($meal['days'] > 0) {
			$data['haodian_endtime'] = TIMESTAMP + intval($meal['days']) * 86400;
		}
		if($meal['price'] > 0) {
			$data['haodian_status'] = 6;//待入驻
		}
	...
	$result = array(
		...
		'config' => array(
			'qualification_verify_status' => intval($config_store['settle']['qualification_verify_status']),
			'meal' => $config_haodian['settle']['status'] == 1 ? $config_haodian['settle']['meal'] : array()
		)
	);
