...
/**
 * 首页红包弹出层显示顺序为： 1代理创建的天降红包 2代理创建的发放红包 3平台创建的天降红包 4平台创建的发放红包 5门店优惠券
*/
function superRedpacket_grant_show() {
	global $_W;
	if($_W['is_agent'] && $_W['agentid'] > 0) {
		//代理创建的天降红包
		$data = superRedpacket_gift_redpackets_format($_W['agentid']);
		if(is_array($data)) {
			return $data;
		}
		//代理创建的发放红包
		$data = superRedpacket_grant_redpackets_format($_W['agentid']);
		if(is_array($data)) {
			return $data;
		}
	}
	...
	//平台创建的天降红包
	$data = superRedpacket_gift_redpackets_format();
	if(is_array($data)) {
		return $data;
	}
	//平台创建的发放红包
	$data = superRedpacket_grant_redpackets_format();
	if(is_array($data)) {
		return $data;
	}
	...
	if(!empty($couponshowsids)) {
		...
		if(!empty($showcoupons)) {
			...
			$data = array(
				'type' => 'coupon',
				'page' => $homeshowcoupon['page'],
				'redpackets' => $showcoupons
			);
			return error(0, $data);
		}
	}
...
function superRedpacket_gift_redpackets_format($agentid = 0) {
	global $_W;
	$gift_redpacket = pdo_get('tiny_wmall_superredpacket', array('uniacid' => $_W['uniacid'], 'agentid' => $agentid, 'type' => 'gift', 'status' => 1));
	if(!empty($gift_redpacket)) {
		...
		if($gift_redpacket['starttime'] <= TIMESTAMP && $gift_redpacket['endtime'] > TIMESTAMP) {//活动时间
			...
			if($grant_object[$today]['grant_success'] < $gift_params['everyday_nums']) {//每日数量
					...
					if($can_grant == 1) {
						foreach($gift_redpacket['data']['redpackets'] as $redpacket) {
							...
							$params = array(
								...
								'agentid' => $gift_redpacket['agentid'],
...
function superRedpacket_grant_redpackets_format($agentid = 0) {
	global $_W;
	$activity_id = pdo_fetchcolumn('select activity_id from ' . tablename('tiny_wmall_activity_redpacket_record') . ' where uniacid = :uniacid and agentid = :agentid and uid = :uid and channel = :channel and type = :type and status = 1 and is_show = 0', array(':uniacid' => $_W['uniacid'], ':agentid' => $agentid, ':uid' => $_W['member']['uid'], ':channel' => 'superRedpacket', ':type' => 'grant'));
