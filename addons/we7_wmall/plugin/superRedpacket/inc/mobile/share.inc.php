...
if(!$_W['ispost']) {
	...
	$_share = array(
		...
		'link' => imurl('superRedpacket/share/index', array('order_id' => $order_id), true)
	);
}
...
if($op == 'index') {
	...
	if($_W['ispost']) {
		...
		if(!empty($is_exist)) {
			imessage(error(-1, '该手机号已绑定其他用户的微信'), ireferer(), 'ajax');
		}
		$params = array(
			'openid' => $openid,
			'nickname' => $_GPC['fans']['nickname'],
			'headimgurl' => $_GPC['fans']['headimgurl'],
			'sex' => $_GPC['fans']['sex'],
			'mobile' => $mobile,
		);
		mload()->model('member');
