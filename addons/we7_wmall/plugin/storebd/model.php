...
function icheckstorebduser($force = true) {
	...
	if($force) {
		...
		imessage(error(-1, '您不是店铺推广员，请联系平台进行添加'), '', 'ajax');
	}
}

function storebd_user_fetch($value, $field = 'id') {
	global $_W;
	//$storebd_user = pdo_get('tiny_wmall_storebd_user', array('uniacid' => $_W['uniacid'], $field => trim($value)));
...
function storebd_user_order_commision($order) {
	global $_W;
	$storebd = pdo_get('tiny_wmall_storebd_store', array('uniacid' => $_W['uniacid'], 'sid' => $order['sid']), array('bd_id', 'fee_takeout', 'fee_instore'));
	...
	} elseif($config_commision['type'] == 2) {
...
function storebd_user_commission_stat($bd_id) {
	...
	return array(
		...
		'getcash_apply' => $commission_getcash_apply,
		'getcash_success' => $commission_getcash_success,
	);
}
...
function storebd_user_getcash_transfers($id) {
	...
	//$storebd_user = pdo_get('tiny_wmall_storebd_user', array('uniacid' => $_W['uniacid'], 'id' => $log['bd_id']));
	...
	$params = array(
		...
		're_user_name' => $storebd_user['title'],
		'amount' => $log['final_fee'] * 100,
		'desc' => "{$storebd_user['title']}" . date('Y-m-d H:i', $log['addtime']) . "店铺推广佣金提现申请"
	);
	mload()->classs('wxpay');
...
function sys_notice_storebd_user_getcash($bd_id, $getcash_log_id , $type = 'apply', $note = '') {
	...
	if($type == 'apply') {
		if(!empty($storebd_user['openid'])) {
			...
			$remark = array(
				"申请　人: " . $storebd_user['title'],
				"手机　号: " . $storebd_user['mobile'],
				"手续　费: " . $log['take_fee'],
				"实际到账: " . $log['final_fee'],
				$note
			);
