...
elseif($op == 'application'){
	$config = get_plugin_config('storebd.basic');
	//$storebd_user = pdo_get('tiny_wmall_storebd_user', array('uniacid' => $_W['uniacid'], 'id' => $_W['storebd_user']['id']));
	...
	if($_W['ispost']) {
		...
		if($get_fee > $storebd_user['credit2']) {
		...
		if($config['fee_getcash']['get_cash_fee_max'] > 0) {
		...
		$data = array(
			...
			'account' => iserializer(
				array(
					'realname' => $storebd_user['realname'],
					'openid' => $openid,
					'avatar' => $storebd_user['avatar'],
					'nickname' => $storebd_user['nickname'],
					'openid_wxapp' => $storebd_user['openid_wxapp'],
				)
			),
			'status' => 2,
			'addtime' => TIMESTAMP,
		...
		} elseif($config['fee_getcash']['get_cash_period'] == 1) {
