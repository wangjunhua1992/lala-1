		...
		foreach($orders as &$value) {
			...
			if($value['spread1'] == $value['spreadid']) {
				...
				if($basic['commission_from'] == 1 && $value['data']['spread']['commission']['commission1_type'] == 'ratio') {
					if($value['plateform_serve_fee'] >= 0) {
			...
			if($value['status'] == $router['status_cancel']) {
				$value['commission'] = 0;
				$value['status_cn'] = '已取消';
			} elseif($value['status'] == $router['status_end'] || $value['status'] == $router['status_end1']) {
				$value['status_cn'] = '已完成';
			} else {
				$value['status_cn'] = '未完成';
			}
...
elseif($op == 'detail') {
	...
	} elseif($order['spread2'] == $_W['member']['uid']) {
	...
	if($order['status'] == $router['status_cancel']) {
		...
		$order['status_cn'] = '已取消';
	} elseif($order['status'] == $router['status_end'] || $order['status'] == $router['status_end1']) {
		$order['status_cn'] = '已完成';
	} else {
		$order['status_cn'] = '未完成';
	}
