...
function spread_group_update($spread_id = 0, $wx_tpl = false) {
	...
	if($relate['group_update_mode'] == 'order_money') {
	...
	} elseif($relate['group_update_mode'] == 'order_money_1') {
		//一级推广订单金额满
		...
		$result = $result + $spread_paotui + $spread_gohome;
	} elseif($relate['group_update_mode'] == 'order_count') {
		//推广订单总数满
		$condition .= " and is_pay = 1 and status = 5 and (spread1 = :spread or spread2 = :spread)";
		$params[':spread'] = $spread_id;
		...
		$result = $result + $spread_paotui + $spread_gohome;
	} elseif($relate['group_update_mode'] == 'order_count_1') {
		//一级推广订单总数满
		$condition .= " and is_pay = 1 and status = 5 and spread1 = :spread";
		$params[':spread'] = $spread_id;
		$result = pdo_fetchcolumn('select count(*) from' . tablename('tiny_wmall_order'). $condition, $params);
		...
		$result = $result + $spread_paotui + $spread_gohome;
	} elseif($relate['group_update_mode'] == 'self_order_money') {
		//自购订单金额满
		...
		$result = $result + $spread_paotui + $spread_gohome;
	} elseif($relate['group_update_mode'] == 'self_order_count') {
		//自购订单数量满
		$condition .= " and is_pay = 1 and status =5 and uid = :uid";
		$params[':uid'] = $spread_id;
		$result = pdo_fetchcolumn('select count(*) from'. tablename('tiny_wmall_order') . $condition, $params);
		...
		$result = $result + $spread_paotui + $spread_gohome;
	} elseif($relate['group_update_mode'] == 'down_count') {
		//下线总人数满
	...
	} elseif($relate['group_update_mode'] == 'down_count_1') {
		//一级总人数满
...
function sys_notice_spread_down($uid, $type, $extra = array()) {
	...
	$send = array(
		...
		'keyword2' => array(
			'value' => date('Y-m-d H:i', $member['addtime']),
			'color' => '#ff510',
		),
		'remark' => array(
			'value' => $remark,
...
function spread_order_balance($order_id, $type = 'takeout') {
	...
	if($spread['valid_period'] == 'once') {
		//清空当前推广上线，设置首次扫码属于清空的没有判断，可在member_spread_bind判断spreadfixed判断是否首次扫码
	...
	if($config_settle['balance_condition'] == 2) {
		...
		if($type == 'takeout' && empty($order['oid'])) {
			$balance = 0;
		} elseif($type == 'gohome' && $order['status'] != 6) {
			$balance = 0;
		}
	}
	...
	if($balance == 1) {
		if(!empty($commission)) {
			if($config_spread['basic']['commission_from'] == 1) {//按平台抽佣计算，需重新计算
				if($order['plateform_serve_fee'] > 0) {
			...
			if($fee_spread1 >= 0) {
				...
				if($commission['commission1_type'] == 'ratio') {
					$extra['remark'] = "{$router['order_type_cn']} 订单号:{$order['id']}, 一级下线佣金费率:{$commission['spread1_rate']}, 佣金:{$fee_spread1}{$_W['Lang']['dollarSignCn']}";
				} elseif($commission['commission1_type'] == 'fixed') {
					$extra['remark'] = "{$router['order_type_cn']} 订单号:{$order['id']}, 一级下线佣金:{$fee_spread1}{$_W['Lang']['dollarSignCn']}";
				}
			...
			if($fee_spread2 >= 0) {
				...
				if($commission['commission2_type'] == 'ratio') {
					$extra['remark'] = "{$router['order_type_cn']} 订单号:{$order['id']}, 二级下线佣金费率:{$commission['spread2_rate']}, 佣金:{$fee_spread2}{$_W['Lang']['dollarSignCn']}";
				} elseif($commission['commission2_type'] == 'fixed') {
					$extra['remark'] = "{$router['order_type_cn']} 订单号:{$order['id']}, 二级下线佣金:{$fee_spread2}{$_W['Lang']['dollarSignCn']}";
				}
...
function member_spread_bind() {
	...
	//4:不限制， 1：新顾客并且没有被推广， 2：新顾客， 3：没有被推广
	...
	} elseif($group_become_child_limit == 2) {
		if(empty($_W['member']['is_mall_newmember'])) {
			return error(-1, '您不是新顾客，不能被推广1');
		}
		if($config_spread_basic['paotui_status'] == 1) {
			$paotui_order = pdo_get('tiny_wmall_errander_order', array('uniacid' => $_W['uniacid'], 'uid' => $_W['member']['uid']), array('id'));
			if(!empty($paotui_order)) {
	...
	} elseif($group_become_child_limit == 3) {
		if(!empty($_W['member']['spread1']) || !empty($_W['member']['spread2'])) {
			return error(-1, '您已经有推广上线，不能被推广');
		}
	}
	...
	$update = array(
		'spread1' => $spread['uid'],
		'spread2' => $config_spread_basic['level'] == 2 ? $spread['spread1'] : 0,
		'spreadfixed' => 1,
	);
...
function order_spread_commission_calculate($type, $data) {
	...
	if(!empty($_W['member']['spread1']) && $_W['member']['spreadfixed'] == 1) {
		...
		if(!empty($spreads)) {
			...
			$condition = ' where uniacid = :uniacid and uid =:uid and is_pay = 1 and status <= :status';
			$params = array(
				':uniacid' => $_W['uniacid'],
				':uid' => $_W['member']['uid'],
				':status' => $router['end_status']
			);
			$order = pdo_fetchcolumn('select count(*) from ' . tablename($router['table']) . $condition, $params);
			if(empty($order)) {
				$is_first = true;
			}
			...
			if(!empty($group1)) {
				...
				if(!empty($is_first)) {
					$commission1_type = $group1['data'][$type]['first_order_return_type'];
					$commission1_value = $group1['data'][$type]['first_order_return_bill1'];
				} else {
			...
			if(!empty($spread2)) {
				...
				if(!empty($group2)) {
					...
					if(!empty($is_first)) {
						$commission2_type = $group1['data'][$type]['first_order_return_type'];
						$commission2_value = $group1['data'][$type]['first_order_return_bill2'];
					} else {
