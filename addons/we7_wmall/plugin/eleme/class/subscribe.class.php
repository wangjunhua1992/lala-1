	...
	public function parse() {
		...
		if($type == 10) {
			...
			$order = array(
				...
				'order_type' => 1,
				'elemeOrderId' => $message['orderId'],
				'ordersn' => $message['orderId'],
				'address' => $message['address'],
				'addtime' => strtotime($message['activeAt']),
				'paytime' => strtotime($message['activeAt']),
				'note' => $message['description'],
				'invoice' => $message['invoice'],
				'mobile' => $message['phoneList'][0],
				'total_fee' => $message['originalPrice'],
				'final_fee' => $message['totalPrice'],
				'store_final_fee' => 0,
				'eleme_store_final_fee' => $message['income'],
				'username' => $message['consignee'],
				'sex' => '',
				'location_x' => $deliveryGeo[1],
				'location_y' => $deliveryGeo[0],
				'order_plateform' => 'eleme',
				'order_channel' => 'eleme',
				'delivery_type' => 0,
				'delivery_fee' => $message['deliverFee'],
				'delivery_time' => !empty($message['deliverTime']) ? $message['deliverTime'] : '立即配送',
				'is_pay' => 1,
				'pay_type' => $message['onlinePaid'] ? 'eleme' : 'delivery',
				'serial_sn' => $message['daySn'],
				'plateform_serve_rate' => $message['serviceRate'],
				'plateform_serve_fee' => $message['serviceFee'],
				'box_price' => $message['packageFee'],
				'plateform_discount_fee' => $message['elemePart'],
				'store_discount_fee' => $message['shopPart'],
				'discount_fee' => $message['activityTotal'],
				'plateform_delivery_fee' => 0,
				'status' => $this->orderStatusTransform($message['status']),
				'stat_year' => date('Y', strtotime($message['activeAt'])),
				'stat_month' => date('Ym', strtotime($message['activeAt'])),
				'stat_day' => date('Ymd', strtotime($message['activeAt'])),
			);
			//vipDeliveryFeeDiscount groups book status refundStatus invoiced hongbao downgraded orderActivities invoiceType taxpayerId
			...
			if(!empty($config_eleme)) {
				if($config_eleme_delivery['delivery_mode'] == 1) {
				...
				} elseif($config_eleme_delivery['delivery_mode'] == 2)  {
					...
					if($config_eleme_delivery['delivery_fee_mode'] == 1) {
						...
						if($distance > $config_eleme_delivery['delivery_price']['start_km']) {
			...
			$eleme = array(
				'fee_type' => 2,
				'fee_rate' => $message['serviceRate'],
				'fee' => $message['serviceFee'],
				'note' => "饿了么服务费率为:{$message['serviceRate']},收取佣金:{$_W['Lang']['dollarSign']}{$message['serviceFee']}"
			);
			...
			if(!empty($account['fee_eleme'])) {
				if($account['fee_eleme']['fee_type'] == 1) {
					...
					$plateform_serve = array(
						'fee_type' => 1,
						'fee_rate' => $account['fee_eleme']['fee_rate'],
						'fee' => $platform_serve_fee,
						'note' => "(饿了么平台最终入账 {$_W['Lang']['dollarSign']}{$message['income']}) x {$account['fee_eleme']['fee_rate']}% {$text}",
						'eleme' => $eleme
					);
				} else {
			...
			if(!empty($message['orderActivities'])) {
				foreach($message['orderActivities'] as $orderActivity) {
					$discount = array(
						...
						'note' => "-{$_W['Lang']['dollarSign']}{$orderActivity['amount']}",
						'fee' => $orderActivity['amount'],
					);
		...
		elseif(in_array($type, array(54))) {
			//配送员已经到店
			order_deliveryer_update_status($order['id'], 'delivery_instore', array('role' => 'eleme'));
		}
		elseif(in_array($type, array(56))) {
			//配送成功
			order_status_update($order['id'], 'end', array('role' => 'eleme'));
		}
		elseif(in_array($type, array(71))) {
			//自行配送
		}
		echo '{"message":"ok"}';
		die;
