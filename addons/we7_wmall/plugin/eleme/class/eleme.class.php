...
class Eleme{
	...
	public function __construct($sid = 0) {
		...
		$this->app = array(
			'sid' => $sid,
			'store' => array(
				'eleme_status' => $store['eleme_status'],
				'eleme' => $store['data']['eleme'],
				'shopid' => $store['elemeShopId'],
			),
			'key' => $this->config['key'],
		...
		$api_urls = array(
			'sandbox' => 'https://open-api-sandbox.shop.ele.me/',
			'open' => 'https://open-api.shop.ele.me/',
		);
	...
	public function getAccessTokenByCode($code, $callback) {
		...
		$extra = array(
			'Authorization' => "Basic " . base64_encode(urlencode($this->app['key']) . ":" . urlencode($this->app['secret'])),
		);
	...
	public function orderStatusTransform($status, $type = 0) {
		$rounter = array(
			...
			'invalid' => 6,
			'settled' => 5,
			'refunding' => 6 //退款中， 应该为6, 在反转数据的时候，这个状态会有问题
		);
		if(empty($type)) {
