...
function svip_buy_notice($id, $extra=array()) {
	...
	if($order_channel == 'wechat') {
		if(!empty($order['openid'])) {
			...
			$remark = array(
				"会员套餐: {$meal['title']}",
				"支付费用: {$order['final_fee']}{$_W['Lang']['dollarSignCn']}",
				"支付方式: " . $order['pay_type_cn'],
				"会员期限: " . date('Y-m-d', $order['starttime']) . '~' . date('Y-m-d', $order['endtime']),
			);
			$params = array(
				'first' => $title,
...
function svip_meal_order_update($id, $type, $extra) {
	...
	if($type == 'pay') {
		...
		$update = array(
			...
			'paytime' => TIMESTAMP,
			'starttime' => TIMESTAMP,
			'endtime' => TIMESTAMP + $order['data']['days'] * 86400,
		);
		...
		$update_member = array(
			'svip_status' => 1,
			'svip_starttime' => TIMESTAMP,
			'svip_endtime' => $update['endtime'],
		);
...
function svip_redpacket_fetchall($filter = array()) {
	...
	if(!empty($redpackets)) {
		...
		foreach($redpackets as &$redpacket) {
			...
			if($filter['get_activity'] == 1 && $redpacket['sid'] > 0) {
			...
			if(!empty($redpacket['data']['times'])) {
				...
				foreach($redpacket['data']['times'] as $rtime) {
					$times[] = "{$rtime['start_hour']}~{$rtime['end_hour']}";
				}
				$times_str = implode(',', $times);
				$limit[] = "限{$times_str}使用";
			}
			if($redpacket['sid'] == 0 && $redpacket['data']['scene'] == 0 && $redpacket['data']['order_type_limit'] > 0) {
...
function svip_redpacket_exchage($redpacketOrId, $exchange_cost = 0) {
	...
	if($redpacket['status'] == 2 || $redpacket['starttime'] > TIMESTAMP) {
		return error(-1, '该红包活动还未开始');
	}
	//若不是奖励金兑换，检测顾客本月已领取了几个红包
	if($exchange_cost != 1) {
		...
		if($num_member >= $exchange_max) {
			return error(-1, '您本月领取次数已达上限');
		}
	}
	//检测要兑换的红包今日发放的数量是否已达上限
	$num_redpacket = svip_redpacket_day_exchange_num($redpacket);
	if($num_redpacket >= $redpacket['amount']) {
		return error(-1, '该红包的今日领取次数已达上限');
	}
	//兑换是否需要奖励金
	$channel = 'svip';
	if($exchange_cost == 1 && $redpacket['exchange_cost'] > 0 && $redpacket['can_exchange'] == 1) {
		...
		$minus = svip_member_svip_credit1_update($_W['member']['uid'], -$redpacket['exchange_cost'], $title, $remark);
		if(is_error($minus)) {
			return $minus;
		}
	...
	$data = array(
		...
		'channel' => $channel,
		'type' => 'grant',
		'scene' => $redpacket['data']['scene'] == 1 ? 'paotui' : 'waimai',
		'order_type_limit' => intval($redpacket['data']['order_type_limit']),
		'discount' => $redpacket['discount'],
		'days_limit' => $redpacket['use_days_limit'],
		'condition' => $redpacket['condition'],
		'uid' => $_W['member']['uid'],
		'activity_id' => $redpacket['id'],
		'sid' => $redpacket['sid'],
		'discount_bear' => $redpacket['data']['discount_bear'],
		'times_limit' => iserializer($times_limit),
		'category_limit' => implode('|', $category_limit)
	);
...
function svip_present_month() {
	...
	} elseif($svip_month > $present_month) {
...
function svip_task_types($type = '') {
	$types = array(
		'oneChargeFee' => array(
			...
			'css' => 'label label-success',
			'achieve_text' => '去充值',
			'achieve_link' => '/pages/member/recharge',
		),
		'oneOrderFee' => array(
			'type' => 'oneOrderFee',
			'title' => '单笔外卖满额',
			...
			'achieve_text' => '去完成',
			'achieve_link' => '/pages/home/index',
		),
		'oneErranderFee' => array(
			'type' => 'oneErranderFee',
			'title' => '单笔跑腿满额',
			'css' => 'label label-warning',
			'achieve_text' => '去完成',
			'achieve_link' => '/pages/paotui/guide',
		)
	);
	if(!empty($type)) {
		return $types[$type];
		...
		if(!empty($tasks)) {
			...
			foreach($tasks as &$val) {
				...
				if(!empty($is_going)) {
					...
					$val['button'] = array(
						'text' => $task_type['achieve_text'],
						'link' => $task_type['achieve_link'],
						'link_type' => 1
					);
				} else {
					$val['button'] = array(
...
function svip_task_takepart_check($taskOrId) {
	...
	if($task_takepart_type > 0) {
		...
		if($task_takepart_type == 1) {//不可重复，包含任务失败
			if($is_exist) {
				return error(-1, '您已参与过该任务，不能再次参加');
			}
		} elseif($task_takepart_type == 2) {//
			if(!empty($is_exist)) {
				if($is_exist['status'] == 1) {
					return error(-1, '您已领取该任务，请先完成后才可再次领取');
				} elseif($is_exist['status'] == 2 || $is_exist['status'] == 3) {
...
function svip_task_finish_check($uid, $task_type, $order=array()) {
	...
	if($task_endtime_type > 0 && TIMESTAMP > $records['overtime']) {
...
//用于订单确认页查询一个svip红包
function svip_store_useful_redpacket_get($storeOrSid, $discount_max = 0, $extra=array()) {
	global $_W;
	...
	if(empty($sid) || empty($store) || empty($discount_max)) {
		return false;
	}
	$cids = array($store['cate_parentid1'], $store['cate_childid1'], $store['cate_parentid2'], $store['cate_childid2']);
	...
	if(!empty($store_redpacket)) {
		...
		if($num_taking > 0) {
			//查询svip_redpacket表红包金额小于门店红包且金额最小的平台红包
			$plateform_redpacket = svip_plateform_redpacket_get($store_redpacket['discount'], 'asc', array('scene' => 'waimai', 'cids' => $cids));
		...
		if($num_taking > 0) {
			$plateform_redpacket = svip_plateform_redpacket_get($discount_max, 'desc', array('scene' => 'waimai', 'cids' => $cids));
			if(!empty($plateform_redpacket)) {
...
function svip_store_redpacket_exchange($storeRedpacketId, $plateformRedpacketId) {
	...
	if($num_redpacket >= $redpacket_store['amount']) {
		return error(-1, '该红包的今日领取次数已达上限');
	}
	...
	//发放新红包
	$data = array(
		'title' => $redpacket_store['title'],
		'channel' => 'svip',
		'type' => 'grant',
		'discount' => $redpacket_store['discount'],
		'days_limit' => $redpacket_store['use_days_limit'],
		'condition' => $redpacket_store['condition'],
		'uid' => $_W['member']['uid'],
		'activity_id' => $redpacket_store['id'],
		'sid' => $redpacket_store['sid'],
		'discount_bear' => $redpacket_store['data']['discount_bear'],
	);
	mload()->model('redPacket');
	$status = redPacket_grant($data, false);
	return $status;
...
function svip_plateform_redpacket_get($discount_max, $discountOrder = 'asc', $extra = array()) {
	global $_W;
	//获取所有的平台红包, 返回符合条件的金额最大或者最小的一个
	$condition = " where uniacid = :uniacid and status = :status and sid = :sid and discount < :discount and `condition` <= :discount and endtime > :endtime and starttime <= :starttime";
	$params = array(
		':uniacid' => $_W['uniacid'],
		':status' => 1,
		':sid' => 0,
	...
	$redpackets = pdo_fetchall("select * from " . tablename('tiny_wmall_svip_redpacket') . " {$condition} order by discount {$discountOrder}, endtime desc ", $params);
	$redpacket = array();
	if(!empty($redpackets)) {
		foreach($redpackets as $val) {
			$val['data'] = iunserializer($val['data']);
			if(($extra['scene'] == 'waimai' && $val['data']['scene'] == 1) || ($extra['scene'] == 'paotui' && $val['data']['scene'] == 0)) {
				continue;
			}
			if(!empty($extra['cids']) && !empty($val['data']['categorys'])) {
				$cids_limit = array_column($val['data']['categorys'], 'id');
				$cids_limit = array_unique($cids_limit);
				$check = array_intersect($cids_limit, $extra['cids']);
				if(empty($check)) {
					continue;
				}
			}
			...
			$val['scene'] = $val['data']['scene'] == 1 ? 'paotui' : 'waimai';
			$val['category_limit'] = implode('|', $cids_limit);
			$redpacket = $val;
			break;
		}
	}
...
function svip_before_timeout_notice() {
	...
	if(!empty($svip_members)) {
		...
		foreach($svip_members as $item) {
			$params = array(
				'first' => "{$item['nickname']}，您的【{$config['title']}】会员即将到期，到期后不再享有会员特权，记得续费哦~",
				'keyword1' => "会员变动",
				'keyword2' => "",
				'keyword3' => date('Y-m-d H:i', TIMESTAMP),
				'keyword4' => "您的会员即将到期",
				'remark' => implode("\n", array(
					"到期时间:" . date('Y-m-d', $item['svip_endtime']),
					"感谢您对{$config['title']}平台的支持与厚爱,期待您继续成为我们的会员。点击续费>>"
				))
			);
			$send = sys_wechat_tpl_format($params);
...
function svip_code_exchange($code, $uid) {
	...
	$update_member = array(
		'svip_status' => 1,
		'svip_starttime' => TIMESTAMP,
		'svip_endtime' => TIMESTAMP + $svip_code['days'] * 86400,
	);
	if($member['svip_endtime'] >= TIMESTAMP) {
		$update_member['svip_starttime'] = $member['svip_starttime'];
