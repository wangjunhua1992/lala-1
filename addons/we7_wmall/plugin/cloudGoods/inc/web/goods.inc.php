...
if($op == 'post') {
	...
	if($_W['ispost']){
		...
		$ids = implode(',', $ids);
		$data = pdo_query('delete from ' . tablename('tiny_wmall_cloudgoods_goods_options') . " WHERE uniacid = :aid AND goods_id = :goods_id and id not in ({$ids})", array(':aid' => $_W['uniacid'], ':goods_id' => $id));
