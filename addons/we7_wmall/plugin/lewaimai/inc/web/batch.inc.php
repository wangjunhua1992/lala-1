...
if($op == 'batch'){
	...
	$goodsArr = array(
		'lwm_sess_token' => $data['lwm_sess_token'],
		'lwm_appid' => $data['lwm_appid'],
	...
	foreach ($stores as &$value) {
		...
		$insert = array(
			...
			'sailed' => $value['xiaoliang'],
			'score' => $value['commentgrade'],
			'cid' => $cache_category[$value['type_id']],
			'is_in_business' => 1,
			'delivery_mode' => 2,
		);
		...
		$store_update = array(
			'business_hours' => iserializer($hour),
			'notice' => $results['shop_notice']
		);
