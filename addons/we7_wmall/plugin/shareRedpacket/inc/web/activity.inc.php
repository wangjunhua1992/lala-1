...
if($op == 'config') {
	...
	if($_W['ispost']) {
		...
		$data = array(
			'title' => $title,
			'uniacid' => $_W['uniacid'],
			'share_redpacket_condition' => floatval($_GPC['share_redpacket_condition']),
			'share_redpacket_min' => floatval($_GPC['share_redpacket_min']),
			'share_redpacket_max' => floatval($_GPC['share_redpacket_max']),
			'share_redpacket_days_limit' => intval($_GPC['share_redpacket_days_limit']),
			'follow_redpacket_min' => floatval($_GPC['follow_redpacket_min']),
			'follow_redpacket_max' => floatval($_GPC['follow_redpacket_max']),
			'follow_redpacket_days_limit' => intval($_GPC['follow_redpacket_days_limit']),
		...
		imessage(error(0, '设置红包活动成功'), ireferer(), 'ajax');
	}
