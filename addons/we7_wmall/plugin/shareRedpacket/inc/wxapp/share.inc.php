...
if($op == 'invite') {
	if($_W['ispost']) {
		...
		$insert = array(
			...
			'share_uid' => $uid,
			'follow_uid' => $_W['member']['uid'],
			'share_redpacket_condition' => $redPacket['share_redpacket_condition'],
			'share_redpacket_discount' => $share_redpacket,
			'share_redpacket_days_limit' => $redPacket['share_redpacket_days_limit'],
			'follow_redpacket_condition' => 0,
			'follow_redpacket_discount' => $follow_redpacket,
			'follow_redpacket_days_limit' => $redPacket['follow_redpacket_days_limit'],
			'addtime' => TIMESTAMP
		);
		...
		$params = array(
			'activity_id' => $redPacket['id'],
			'title' => '新用户专享红包',
			'channel' => 'shareRedpacket',
			'type' => 'mallMewMember',
			'uid' => $_W['member']['uid'],
			'discount' => $follow_redpacket,
			'condition' => 0,
			'days_limit' => $redPacket['follow_redpacket_days_limit'],
		);
	...
	$result = array(
		'redPacket' => $redPacket,
		'member' => $member,
		'mall_title' => $_W['we7_wmall']['config']['mall']['title'],
