...
if($op == 'stick') {
	...
	if($_W['ispost']) {
		...
		foreach($displayorders as $key =>$val) {
			...
			if(!empty($val)) {
				...
				foreach($fee as $k => $v) {
					$v = explode('-', $v);
					$displayorder_fees[$val]['fees'][$v[0]] = array(
						'day' => $v[0],
						'fee' => $v[1],
					);
				}
		...
		$data = array(
			'status' => intval($_GPC['status']) ? intval($_GPC['status']) : 0,
			'num' => intval($_GPC['num']),
			'displayorder_fees' => $displayorder_fees
		);
	...
	foreach($fees as $key =>$val) {
		...
		$displayorder['displayorder_fees'][] = array(
			'fees' => implode(',', $displayorder[$key]),
