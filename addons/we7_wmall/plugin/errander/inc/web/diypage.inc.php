...
elseif($op == 'post') {
	...
	if($_W['ispost']) {
		...
		$insert = array(
			'uniacid' => $_W['uniacid'],
			'scene' => $_GPC['data']['page']['scene'],
			'name' => $_GPC['data']['page']['name'],
			'data' => base64_encode(json_encode($_GPC['data'])),
			'agreement' => $agreement,
			'thumb' => trim($_GPC['data']['page']['thumb']),
			'start_hour' => trim($_GPC['data']['page']['start_hour']),
			'end_hour' => trim($_GPC['data']['page']['end_hour']),
		);
		if(!empty($id)) {
			$insert['updatetime'] = TIMESTAMP;
