...
if($op == 'post') {
	...
	if($id > 0) {
		...
		if(empty($item)) {
			imessage(error(-1, '跑腿类型不存在或已删除'), ireferer(), 'ajax');
		}
	...
	if($_W['ispost']) {
		...
		$data = array(
			...
			'distance_calculate_type' => trim($_GPC['distance_calculate_type']),
			'weight_fee_status' => intval($_GPC['weight_fee_status']),
			'goods_value_status' => intval($_GPC['goods_value_status']),
		...
		if($data['type'] == 'multiaddress') {
			...
			if(empty($multiaddress['max'])) {
				imessage(error(-1, '请设置最多可添加几个地址'), ireferer(), 'ajax');
			}
			...
			if(count($multiaddress['fee']) != $multiaddress['max']) {
				imessage(error(-1, '地址数量和配送费数量不一致'), ireferer(), 'ajax');
			}
		...
		if(!empty($_GPC['labels'])){
			foreach($_GPC['labels']['name'] as $key => $val){
				...
				$labels[] = array(
					'icon' => trim($_GPC['labels']['icon'][$key]),
					'name' => trim($_GPC['labels']['name'][$key]),
					'autosearch' => intval($_GPC['labels']['autosearch'][$key]),
					'keywords' => trim($_GPC['labels']['keywords'][$key]),
				);
			}
		}
		...
		$data['notice'] = array(
			'text' => trim($_GPC['text']),
			'url' => trim($_GPC['url']),
			'btn_text' => trim($_GPC['btn_text']) ? trim($_GPC['btn_text']) : '知道了',
			'between_time' => intval($_GPC['between_time']) ? intval($_GPC['between_time']) : 0
		);
