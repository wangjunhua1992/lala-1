...
if($op == 'post') {
	...
	if($_W['ispost']) {
		...
		if($_config_plugin['serve_radius'] > 0 && $distance > ($_config_plugin['serve_radius'] * 1000)) {
...
if($op == 'suggestion') {
	...
	if(!empty($result['tips'])) {
		$distance_sort = 0;
		if($_config_plugin['serve_radius'] > 0 && !empty($_config_plugin['map']['location_x']) && !empty($_config_plugin['map']['location_y'])) {
			$distance_sort = 1;
		}
		foreach($result['tips'] as $key => &$val) {
			...
			if(is_array($val['location'])) {
				...
				if($distance_sort == 1) {
					...
					if($val['distance'] > $_config_plugin['serve_radius']) {
