...
if($op == 'list') {
	...
	//订单统计
	$condition = ' where uniacid = :uniacid and agentid = :agentid and status = 3 and is_pay = 1 and stat_day = :stat_day';
...
if($op == 'detail') {
	...
	if(empty($order)) {
		imessage('订单不存在或已经删除', ireferer(), 'error');
	}
	if($order['discount_fee'] > 0) {
		$discount = errander_order_fetch_discount($id);
	}
	$pay_types = order_pay_types();
...
if($op == 'status') {
	...
	if($type == 're_notify_deliveryer_collect') {
		$status = errander_order_status_update($order_id, $type);
