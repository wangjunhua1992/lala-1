...
if($op == 'post') {
	...
	if(!empty($id)) {
		$superRedpacket = pdo_fetch('select * from ' . tablename('tiny_wmall_superredpacket') . ' where  uniacid = :uniacid and id = :id', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		if(!empty($superRedpacket)) {
			$superRedpacket['data'] = json_decode(base64_decode($superRedpacket['data']), true);
		}
	} else {
		$superRedpacket_yes = pdo_fetch('select id from ' . tablename('tiny_wmall_superredpacket') . ' where uniacid = :uniacid and agentid = :agentid and type = :type and status = 1', array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid'], ':type' => 'gift'));
		if(!empty($superRedpacket_yes)) {
			imessage('已创建天降红包活动, 如需重新添加天降红包活动，请先撤销其他活动', ireferer(), 'info');
		}
	}
...
if($op == 'del') {
	...
	foreach($ids as $id) {
		pdo_delete('tiny_wmall_superredpacket', array('uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], 'id' => $id));
	}
...
if($op == 'cancel') {
	...
	imessage(error(0, '撤销成功'), ireferer(), 'ajax');
}