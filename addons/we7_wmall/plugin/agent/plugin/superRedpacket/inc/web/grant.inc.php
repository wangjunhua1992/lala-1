...
elseif($op == 'post') {
	...
	if($_W['ispost']) {
		...
		if($data['customer']['type'] == 1) {
			...
			//近30天成交顾客
			$condition = ' where uniacid = :uniacid and agentid = :agentid and status = 5 and is_pay = 1 and addtime > :addtime';
			$params = array(
				':uniacid' => $_W['uniacid'],
				':agentid' => $_W['agentid'],
				':addtime' => TIMESTAMP - 30 * 86400
			);
			$uid = pdo_fetchall('select uid from ' . tablename('tiny_wmall_order') . $condition, $params, 'uid');
			$uid = array_keys($uid);
		}
		if(empty($uid)) {
