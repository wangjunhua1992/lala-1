...
elseif($op == 'batch_is_in_business') {
	...
	$cache_key = "{$_W['uniacid']}:{$_W['agentid']}:stores_is_business";
	if($value == 1) {
		...
		if(empty($stores_is_business) || (!empty($stores_is_business['expire']) && empty($stores_is_business['data']))) {
			//缓存数据不存在获取休息中的店铺
			$stores_is_business = pdo_getall('tiny_wmall_store', array('uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], 'status' => 1, 'is_in_business' => 0), array('id', 'is_in_business'));
		}
		...
		//缓存所有的营业店铺
		$stores_is_business = pdo_getall('tiny_wmall_store', array('uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], 'status' => 1, 'is_in_business' => 1), array('id', 'is_in_business'));
		icache_write($cache_key, $stores_is_business, 3600);
		pdo_update('tiny_wmall_store', array('is_in_business' => 0), array('uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], 'status' => 1));
	}
...
elseif($op == 'batch_status') {
	$value = intval($_GPC['value']);
	$cache_key = "{$_W['uniacid']}:{$_W['agentid']}:store_status";
	if($value == 1) {
		$stores = icache_read($cache_key);
		if(empty($stores) || (!empty($stores['expire']) && empty($stores['data']))) {
			$stores = pdo_getall('tiny_wmall_store', array('uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], 'status' => 0), array('id', 'status'));
		}
		if(!empty($stores)) {
		...
		$stores = pdo_getall('tiny_wmall_store', array('uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], 'status' => 1), array('id', 'status'));
		icache_write($cache_key, $stores, 3600);
		pdo_update('tiny_wmall_store', array('status' => 0), array('uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], 'status' => 1));
	}
	imessage(error(0, ''), '', 'ajax');
}
