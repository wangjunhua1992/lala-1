...
elseif($op == 'stat') {
	...
	if($_W['isajax'] && $_W['ispost']) {
		...
		$data = pdo_fetchall("SELECT addtime FROM " . tablename('tiny_wmall_order') . 'WHERE uniacid = :uniacid AND agentid = :agentid AND deliveryer_id = :deliveryer_id AND delivery_type = 2 and status = 5 and addtime >= :starttime and addtime <= :endtime', array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid'], ':deliveryer_id' => $id, ':starttime' => $start, ':endtime' => $end));
		foreach($data as $da) {
