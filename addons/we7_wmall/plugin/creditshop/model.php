...
function creditshop_order_update($orderOrId, $type, $extra = array()) {
	...
	if($type == 'pay') {
		...
		} elseif($order['goods_type'] == 'credit2') {
			mload()->model('member');
		...
		if($order['is_pay'] == 0 && $order['status'] == 1 && $order['use_credit1'] > 0 && $order['use_credit1_status'] == 1 && $order['use_credit2'] > 0) {
			mload()->model('member');
...
function creditshop_order_notice($orderOrId, $type, $extra = array()) {
	...
	if($channel == 'wap') {
		if($type == 'pay') {
			$params = array(
				'first' => "您在积分商城兑换了{$goods_types[$order['goods_type']]}",
				'keyword1' => "{$goods_types[$order['goods_type']]}:{$order['goods_info']['title']}",
				'keyword2' => date('Y-m-d H:i', $order['paytime']),
				'keyword3' => '积分商城积分兑换',
			);
		} elseif($type == 'handle') {
			$params = array(
				'first' => '您在积分商城兑换的商品已核销',
				'keyword1' => "商品:{$order['goods_info']['title']}",
				'keyword2' => date('Y-m-d H:i', TIMESTAMP),
				'keyword3' => '积分兑换商品已核销',
			);
		}
		...
		if($config_wxapp_basic['tpl_consumer_url'] == 'wxapp') {
			$miniprogram = array(
				...
				'pagepath'=> "pages/creditshop/detail?id={$order['id']}",
			);
		}
		...
		if($type == 'pay') {
			$send = array(
				...
				'phrase9' => array(
					'value' => $status_cn[$order['status']],
				),
				'thing2' => array(
					'value' => "类型:积分商城订单,消耗:{$order['use_credit1']}积分",
				),
				'amount1' => array(
		...
		$status = $acc->sendTplNotice($order['openid'], $_W['we7_wmall']['config']['wxapp']['wxtemplate']['public_tpl'], $send, "pages/creditshop/detail?id={$order['id']}");
	}
	if(is_error($status)) {
...
function creditshop_order_manager_notice($orderOrId, $type, $extra = array()) {
	...
	if($type == 'pay') {
		...
		$params = array(
			...
			'keyword1' => "{$goods_types[$order['goods_type']]}:{$order['goods_info']['title']}",
			'keyword2' => date('Y-m-d H:i', $order['paytime']),
			'keyword3' => "积分商城{$goods_types[$order['goods_type']]}兑换",
		);
	}
	$send = sys_wechat_tpl_format($params);
