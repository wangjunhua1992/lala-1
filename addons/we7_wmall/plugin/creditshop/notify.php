...
if(is_error($result)) {
	die(json_encode(array('status' => 'fail', 'errorMessage' => $result['message'], 'credits' => $member['credit1'])));
}
if($_GET['channel'] == 'consume') {
	$order = pdo_get('tiny_wmall_creditshop_order', array('uniacid' =>$_W['uniacid'], 'ordernum' => $_GPC['orderMum']));
	if(!empty($order)) {
		die(json_encode(array('status' => 'fail', 'errorMessage' => '订单已存在', 'credits' => $member['credit1'])));
	}

	if($member['credit1'] < $_GPC['credits']) {
		die(json_encode(array('status' => 'fail', 'errorMessage' => '积分不足', 'credits' => $member['credit1'])));
	}
	...
	$order = array(
		...
		'status' => 0,
		'credits' => $_GPC['credits'],
		'itemcode' => $_GPC['itemCode'],
		'actualPrice' => round($_GPC['actualPrice'] / 100, 2),
		'faceprice' => round($_GPC['facePrice'] / 100, 2),
		'description' => $_GPC['description'],
		'addtime' => $_GPC['timestamp'] / 1000,
	);
