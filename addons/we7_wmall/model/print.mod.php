...
function print_add_order($printDeviceInfo = array(), $order = array()) {
	...
	if($printDeviceInfo['type'] == 'feie') {
		...
		$postdata = array(
			'sn' => $printDeviceInfo['deviceno'],
			'key' => $printDeviceInfo['key'],
			'printContent' => $printContent,
			'times' => $printDeviceInfo['times'],
			'backurl' => "{$_W['siteroot']}addons/we7_wmall/payment/print/notify.php"
		);
		if($printDeviceInfo['language'] == 'Uyghur') {
			$postdata['language'] = 'Uyghur';
		};
		$posturl = 'http://api163.feieyun.com/FeieServer/printOrderAction';
	} elseif($printDeviceInfo['type'] == 'feie_new') {
		...
		$postdata = array(
			...
			'times'=> $printDeviceInfo['times'],
			'backurl' => "{$_W['siteroot']}addons/we7_wmall/payment/print/notify.php",
			'callbackurl' => "{$_W['siteroot']}addons/we7_wmall/payment/print/notify.php"
		);
		if($printDeviceInfo['language'] == 'Uyghur') {
			$postdata['language'] = 'Uyghur';
		};
		$posturl = 'http://api.feieyun.cn/Api/Open/';
	} elseif($printDeviceInfo['type'] == 'feiyin') {
		$qrcode = str_replace(array("<QR>", "</QR>"), array('', ''), $printDeviceInfo['content']['qrcode']);
		//$printDeviceInfo['content']['qrcode'] = "<QRcode# size=8>{$qrcode}</QRcode#>";
		...
		$postdata = array(
			'memberCode' => $printDeviceInfo['member_code'],
			'deviceNo' => $printDeviceInfo['deviceno'],
			'reqTime' => number_format(1000*time(), 0, '', ''),
			'msgDetail' => $printDeviceInfo['content'],
			'mode' => 2,
			'msgNo' => $printDeviceInfo['orderindex'],
		);
	...
	} elseif($printDeviceInfo['type'] == 'AiPrint') {
	...
	} elseif($printDeviceInfo['type'] == '365') {
		...
		$postdata = array(
			'deviceNo' => $printDeviceInfo['deviceno'],
			'key' => $printDeviceInfo['key'],
			'printContent' => $printDeviceInfo['content'],
			'times' => $printDeviceInfo['times']
		);

		$posturl = 'http://open.printcenter.cn:8080/addOrder';
	} elseif($printDeviceInfo['type'] == 'yilianyun') {
		...
		$printDeviceInfo['content'] = str_replace(array("<L>", "</L>"), array('<FH>', '</FH>'), $printDeviceInfo['content']);
		...
		$postdata = array(
			'partner' => $printDeviceInfo['member_code'],
			'machine_code' => $printDeviceInfo['deviceno'],
			'sign' => $sign,
			'content' => $printDeviceInfo['content'],
			'time' => $time
		);
		$postdata = http_build_query($postdata);
		$posturl = 'http://open.10ss.net:8888';
	} elseif($printDeviceInfo['type'] == 'qiyun') {
		$printDeviceInfo['content'] = str_replace(array("<C>", "</C>"), array("", ""), $printDeviceInfo['content']);
		$printDeviceInfo['content'] = str_replace(array("<CB>", "</CB>"), array("", ""), $printDeviceInfo['content']);
		$printDeviceInfo['content'] = str_replace(array("<L>", "</L>"), array('', ''), $printDeviceInfo['content']);
	...
	} elseif($printDeviceInfo['type'] == 'xixun'){
		$printDeviceInfo['content'] = str_replace(array("<CB>", "</CB>"), array("", ""), $printDeviceInfo['content']);
		$printDeviceInfo['content'] = str_replace(array("<C>", "</C>"), array(' ', ' '), $printDeviceInfo['content']);
		$printDeviceInfo['content'] = str_replace(array("<L>", "</L>"), array("", ""), $printDeviceInfo['content']);
		...
		$postdata = array(
			'dingdan' => $printDeviceInfo['content'],
			'dingdanID' => $order['print_sn'],
			'dayinjisn' => $printDeviceInfo['deviceno'],
			'pages' => $printDeviceInfo['times'],
			'replyURL' => "{$_W['siteroot']}addons/we7_wmall/payment/print/notify.php"
		);
	} elseif($printDeviceInfo['type'] == '365_s2') {
		$postdata = array(
			'deviceNo' => $printDeviceInfo['deviceno'],
			'key' => $printDeviceInfo['key'],
			'printContent' => implode('<BR>', $printDeviceInfo['content']),
		...
		$posturl = 'http://open.printcenter.cn:8080/addOrder';
	} elseif($printDeviceInfo['type'] == 'jinyun') {
		//v8s1-s4不能打印二维码 v9s1-s8， s2加宽，s3加长，s4加宽加长
		...
		$printDeviceInfo['content'] = str_replace(array("<N>", "</N>"), array('', ''), $printDeviceInfo['content']);
		//$printDeviceInfo['content'] = str_replace(array("<QR>", "</QR>"), array('<AM><QR_T>QR_CODE</QR_T><QR_S>7</QR_S><QR_D>', '</QR_D></AM>'), $printDeviceInfo['content']);
		$print_params = array(
			'seri_num'=> $printDeviceInfo['print_no'],//'788713104204'
			'print_data'=> $printDeviceInfo['content'],
			'print_type'=> '',
		);
		$postdata = array(
			'timestamp'=> TIMESTAMP,
			'action'=> 'print',
			'token'=> $printDeviceInfo['member_code'],//'d0s0x9naLxFr'
			'data'=> base64_encode(json_encode($print_params)),
		);
		ksort($postdata);
		$sign_str='';
		...
		$sign_str .= $printDeviceInfo['api_key'];//'JnnKUX4oLJr1TfH7mAKxzay6hTe1qSs7fOP8LSww';
		$sign_str = md5($sign_str);
		$postdata['sign'] = $sign_str;
		$posturl = 'http://www.jinyunzn.com/api/print/index.php';
	} elseif($printDeviceInfo['type'] == 'lingdian') {
		...
		$printContent = implode('<br/>', $printDeviceInfo['content']);
		$postdata = array(
			'secret' => $printDeviceInfo['key'],
			'print_sn' => $printDeviceInfo['print_no'],
			'print_num' => $printDeviceInfo['times'],
			'order_id' => $order['id'],
			'order_content' => $printContent,
			'notify_url' => ""
		);
	...
	if($printDeviceInfo['type'] == 'feiyin' || $printDeviceInfo['type'] == 'AiPrint') {
		...
		if(in_array($printDeviceInfo['type'], array('feie', '365'))) {
			...
			if($result['responseCode']) {
				...
				if(!$result_query['responseCode']) {//打印和未打印都返回0
					return true;
				}
...
function print_query_printer_status($printer) {
	if($printer['type'] == 'feie') {
		...
		$posturl = 'http://api.feieyun.cn/Api/Open/';
		$response = ihttp_post($posturl, $postdata);
	} elseif($printer['type'] == 'feiyin') {
		$postdata = array(
			'memberCode' => $printer['member_code'],
			'reqTime' => number_format(1000*time(), 0, '', ''),
		);
		...
		$response = ihttp_get($posturl);
	} elseif($printer['type'] == '365' || $printer['type'] == '365_s2') {
		$postdata = array(
			'deviceNo' => $printer['print_no'],
			'key' => $printer['key'],
		);
	...
	if(in_array($printer['type'], array('feie', '365', '365_s2'))) {
		$result = @json_decode($response['content'], true);
	} elseif(in_array($printer['type'], array('feie_new'))) {
		$result = @json_decode($response['content'], true);
		return $result['data'];
	} else {
...
function print_2_Uyghur($content) {
	$data = array(
		'token' => '1b4667f54fawed9f9847665027ssd497c8848a91',
		'text' => $content,
	);
