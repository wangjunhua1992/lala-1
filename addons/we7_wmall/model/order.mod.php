					...
					if(!empty($order['goods']) || $order['order_type'] == 4) {
						...
						if($order['extra_fee'] > 0) {
							foreach($order['data']['extra_fee'] as $item) {
								$content[] = "{$item['name']}:{$item['fee']}{$_W['Lang']['dollarSignCn']}";
							}
						}
						...
						if(!empty($order['data']['member_choice'])) {
							...
							foreach($order['data']['member_choice'] as $choice) {
								$content[] = "{$choice['name']}:{$choice['fee']}{$_W['Lang']['dollarSignCn']}";
							}
							$content[] = '********************************';
						}
						if(!empty($grant)) {
...
function order_status_notice($id, $status, $note = '') {
		...
		if($order_channel == 'wap') {
			...
			if($type == 'pay') {
				...
				$send = array(
					...
					'date9' => array(
						'value' => date('Y-m-d H:i', $order['endtime'])
					),
					'character_string10' => array(
						'value' => $order['ordersn']
					),
					'phrase11' => array(
						'value' => $order['order_type_cn']
				...
				$send = array(
					...
					'thing1' => array(
						'value' => empty($extra['note']) ? '未知' : $extra['note']
					),
					'date2' => array(
			...
			if(in_array($type, array('pay', 'handle', 'delivery_assign', 'end', 'cancel'))) {
				$template_id = $_W['we7_wmall']['config']['wxapp']['wxtemplate']['order'][$type];
				$status = $acc->sendTplNotice($order['openid'], $template_id, $send, "pages/home/index");
			}
		}
	...
	if($order['order_channel'] == 'h5app') {
		...
		if(!empty($token)) {
			...
			if(is_error($status)) {
				slog('customerappJpush', '顾客App推送通知顾客', $status, $status['message']);
			}
		}
...
function order_update_goods_info($order_id, $sid, $cart = array()) {
	...
	if(empty($cart['data'])) {
		slog('ordercart', '订单商品', array('order_id' => $order_id), "订单商品插入失败,订单号：{$order_id},门店id:{$sid}");
		return false;
	}
	if($cart['pindan_id'] > 0 && $cart['pindan_status'] == 2 && !empty($cart['pindan_data'])) {
		$cart['data'] = $cart['pindan_data'];
	}
	$categorys = pdo_getall('tiny_wmall_goods_category', array('uniacid' => $_W['uniacid']), array('id', 'title'), 'id');
...
function order_begin_payrefund($refund_id) {
	...
	if($refund['pay_type'] == 'credit') {
		if($refund['uid'] > 0) {
			$log = array(
				$refund['uid'],
				"外送模块订单退款, 订单号:{$refund['order_sn']}, 退款金额:{$refund['fee']}{$_W['Lang']['dollarSignCn']}",
				'we7_wmall'
			);
			mload()->model('member');
...
function order_status_update($id, $type, $extra = array()) {
	...
	if($type == 'handle') {
		...
		if($extra['channel'] == 're_notify_deliveryer_collect') {
			...
			if(in_array($extra['role'], array('dada', 'uupaotui', 'shansong', 'dianwoda', 'sfexpress'))) {
				$third_partys = array(
					'dada' => '达达',
					'uupaotui' => 'UU跑腿',
					'shansong' => '闪送',
		...
		//当订单已经推送到顺丰同城, 需要取消顺丰同城的订单
		if(get_order_data($order, 'sfexpress.status') == 1 && $extra['role'] !== 'sfexpress'){
			mload()->model('plugin');
			$_W['_plugin'] = array(
				'name' => 'sfexpress'
			);
			pload()->classs('sfexpress');
			$sfexpress = new sfexpress($order['id'], $order['sid']);
			$result = $sfexpress->cancelOrder();
			if(is_error($result)) {
				return $result;
			}
		}
		...
		if(is_open_order($order) || !$order['is_pay'] || $order['final_fee'] <= 0 || ($order['is_pay'] == 1 && ($order['pay_type'] == 'delivery' || $order['pay_type'] == 'cash'))) {
			...
			$note = array(
				"取消原因: {$extra['note']}",
				"退款金额: {$refund_fee}{$_W['Lang']['dollarSignCn']}",
				"已付款项会在1-3工作日内返回您的账号",
			);
			...
			if(!in_array($_W['role'], array('merchanter', 'clerker')) && $order['status'] > 1 && $order['is_pay'] == 1) {
		...
		if($order['is_pay'] == 1) {
			...
			if($fee_yucunjin > 0) {
				store_update_yucunjin($order['sid'], -$fee_yucunjin, 1, $order['id'], "{$order['order_type_cn']}订单扣除");
			}
			if($order['agentid'] > 0) {
				mload()->model('agent');
				$remark = '';
...
function order_assign_deliveryer($order_id, $deliveryer_id, $update_deliveryer = false, $note = '') {
	...
	if(empty($order['clerk_notify_collect_time'])) {
		$update['clerk_notify_collect_time'] = $update['delivery_assign_time'];
	}
	pdo_update('tiny_wmall_order', $update, array('uniacid' => $_W['uniacid'], 'id' => $order['id']));
	pdo_update('tiny_wmall_order_stat', array('status' => 4), array('uniacid' => $_W['uniacid'], 'oid' => $order['id']));
...
function order_system_status_update($id, $type, $extra = array()) {
	...
	if($type == 'pay') {
		...
		if($order['order_type'] <= 2) {
			$reserve_status = order_is_reserve(array_merge($order, array('paytime' => $update['paytime'])));
...
function order_deliveryer_update_status($id, $type, $extra = array()) {
	...
	if($type == 'delivery_assign') {
		...
		if(in_array($extra['role'], array('eleme', 'meituan','dada', 'uupaotui', 'shansong', 'dianwoda', 'sfexpress'))) {
...
function order_calculate_deliveryer_fee($order, $deliveryerOrid = 0) {
	...
	if(!empty($special_deliveryer_takeout) && $special_deliveryer_takeout['status'] == 1) {
		if($special_deliveryer_takeout['type'] == 1 && $special_deliveryer_takeout['fee'] > 0) {
			$plateform_deliveryer_fee += floatval($special_deliveryer_takeout['fee']);
		} elseif($special_deliveryer_takeout['type'] == 2 && $special_deliveryer_takeout['rate'] > 0) {
...
function order_update_bill($order_id, $extra_data = array()) {
	...
	if($fee_config['type'] == 2) {
		...
		$plateform_serve = array(
			...
			'note' => "每单固定{$platform_serve_fee}{$_W['Lang']['dollarSignCn']}"
		);
	} elseif($fee_config['type'] == 3) {
		$plateform_serve_rate = 0;
		$levels = array(
			$fee_config['level1']['fee_start'] => $fee_config['level1'],
			$fee_config['level2']['fee_start'] => $fee_config['level2'],
		);
		krsort($levels);
		...
		$plateform_serve = array(
			...
			'fee_start' => $fee_start,
			'fee' => $platform_serve_fee,
			'note' => "订单满{$fee_start}{$_W['Lang']['dollarSignCn']}抽成{$platform_serve_fee}{$_W['Lang']['dollarSignCn']}"
		);
	} elseif($fee_config['type'] == 4) {
		$platform_serve_fee = 0;
		$note = array(
			'yes' => array(),
			'no' => array(),
		);
		$single_percent = $fee_config['single_percent'];
		unset($single_percent['items_no']);
		$fee_items = store_serve_fee_items();
		foreach($single_percent as $key => $value) {
			if($key == 'delivery_fee' && $order['delivery_type'] == 2) {
				continue;
			}
			$value = floatval($value);
			if($value > 0) {
				$fee_item_name = $fee_items['yes'][$key];
				$fee_item_value = "{$order[$key]}x{$value}%";
				if($key == 'price') {
					if(get_plugin_config('opetools.store.goods_balance_type') == 1) {
						$fee_item_value = "{$order['caigou_price']}x{$value}%";
					}
				}
				$platform_serve_fee += $order[$key] * $value / 100;
				$note['yes'][] = "{$fee_item_name} {$_W['Lang']['dollarSign']}{$fee_item_value}";
			}
		}
		$items_no = array_unique($fee_config['single_percent']['items_no']);
		if(!empty($items_no)) {
			foreach($items_no as $item) {
				$platform_serve_fee -= $order[$item];
				$note['no'][] = "{$fee_items['no'][$item]} {$_W['Lang']['dollarSign']}{$order[$item]}";
			}
		}
		$platform_serve_fee = floatval($platform_serve_fee);
		if($platform_serve_fee < 0) {
			$platform_serve_fee = 0;
		}
			...
			if(!empty($special_goods_ids)) {
				foreach($special_goods_ids as $goodsid) {
					...
					foreach($cart_goods[$goodsid]['options'] as $option) {
						...
						$price_total = floatval($option['total_discount_price']);
		...
		if(!empty($fee_config['items_yes'])) {
			...
			foreach($fee_config['items_yes'] as $item) {
				...
				$fee_item_name = $fee_items['yes'][$item];
				$fee_item_value = $order[$item];
				if($item == 'price') {
					if(get_plugin_config('opetools.store.goods_balance_type') == 1) {
						$order['caigou_price'] -= $special_goods_price_total;
						$fee_item_value = $order['caigou_price'];
					} else {
						$order['price'] -= $special_goods_price_total;
						$fee_item_value = $order['price'];
					}
				}
				$basic += $fee_item_value;
				$note['yes'][] = "{$fee_item_name} {$_W['Lang']['dollarSign']}{$fee_item_value}";
			}
		}
	...
	if(get_plugin_config('opetools.store.goods_balance_type') == 1) {
		$store_order_total_fee = $order['caigou_price'] + $order['box_price'] + $order['pack_fee'] + $order['serve_fee'];
	}
	if($order['order_type'] == 1) {
		if($order['delivery_type'] == 1) {
			//店内配送模式
...
function goods_total_warning_notice($goodsOrid, $optionOrid, $extra = array()) {
	...
	$params = array(
		'first' => $tips,
		'keyword1' => $extra['store']['title'],
		'keyword2' => date('Y-m-d H:i:s'),
		'keyword3' => "商品{$goods['title']}库存不足",
	);
	$send = sys_wechat_tpl_format($params);
	mload()->model('clerk');
...
function order_delivery_info($store, $predict_index = -1, $condition = array()) {
	...
	foreach($delivery_time['times'] as &$time) {
		...
		$time['total_delivery_price_cn'] = "{$_W['Lang']['dollarSign']}{$time['total_delivery_price']}配送费";
	}
	...
	if($store['is_in_business_hours'] && !store_is_in_business_hours($store['business_hours'])) {
		$is_rest_order = 1;
		$rest_order_info = store_rest_start_delivery_time($store);
		$predict_timestamp = strtotime($rest_order_info['delivery_time']) + 60 * $store['delivery_time'];
		if($rest_order_info['nextday'] == 1) {
			//明天配送
			$predict_timestamp = $predict_timestamp + 86400;
			$nextday = 1;
			$delivery_time['nextday'] = date('m-d', $predict_timestamp);
			if(date('m-d', TIMESTAMP) == $delivery_time['days'][0]) {
				unset($delivery_time['days'][0]);
			}
			$delivery_time['days'] = array_values($delivery_time['days']);
		}
	}
	$data = array_order($predict_timestamp, $delivery_time['timestamp']);
	$sys_predict_index = array_search($data, $delivery_time['timestamp']);
	if($nextday == 1) {
		$data = array_order($predict_timestamp - 86400, $delivery_time['timestamp']);
		$sys_predict_index = array_search($data, $delivery_time['timestamp']);
	}
	...
	$delivery_times['predict_year_cn'] = date('Y');
	if(strtotime($delivery_times['predict_year_cn'] . '-' . $delivery_times['predict_day_cn']) < strtotime(date('Y-m-d'))) {
		$delivery_times['predict_year_cn'] += 1;
	}
...
function order_calculate($sidOrStore, $cart, $condition = array()) {
	...
	if($meal_redpacket_key) {
		...
		if($redpacket_id > 0) {
			mload()->model('redPacket');
			$redpacket = redpacket_available_check($redpacket_id, $cart['price'], explode('|', $store['cid']), array('scene' => 'waimai', 'sid' => $sid, 'order_type' => $condition['order_type'], 'agentid' => $store['agentid']));
		}
	}
	if($meal_redpackets) {
	...
	$order = array(
		...
		'member_choice_note' => $member_choice_note,
...
function order_push_dada($id, $push) {
	global $_W;
	$_W['_plugin'] = array(
		'name' => 'dada'
	);
	...
	$dada = new DaDa();
	if(!$push) {
		$result = $dada->queryDeliverFee($id);
	} else {
		$result = $dada->addAfterQuery($id);
	}
	return $result;
}
...
function order_plateform_notice($idOrorder, $type, $extra = array()) {
	...
	if($type == 'ordernew') {
		$pextra = array(
			'title' => '平台有新的外卖订单',
			'voice_text' => "店铺{$store['title']}有新的外卖订单,收货地址为{$order['address']},订单金额{$order['final_fee']}{$_W['Lang']['dollarSignCn']}",
...
//订单推送到顺丰同城
function order_push_sfexpress($id, $push = false) {
	global $_W;
	$_W['_plugin'] = array(
		'name' => 'sfexpress'
	);
	...
	$sfexpress = new sfexpress($id, $order['sid']);
	if($push) {
		$result = $sfexpress->createOrder();
		if(!is_error($result)) {
			$result['status'] = 1;
			set_order_data($id, 'sfexpress', $result);
		}
	} else {
		$result = $sfexpress->preCreateOrder();
	}
	return $result;
}

function order_stat_repair($orderOrId) {
	global $_W, $_GPC;
	$order = $orderOrId;
	if(!is_array($order)) {
	...
	$sid = $order['sid'];
	$is_exist = pdo_fetchcolumn('select count(*) from ' . tablename('tiny_wmall_order_stat') . ' where uniacid = :uniacid and oid = :oid', array(':uniacid' => $_W['uniacid'], ':oid' => $order['id']));
	if(empty($is_exist)) {
		$cart = $order['data']['cart'];
		if(!empty($cart)) {
			unset($cart['88888']);
			$categorys = pdo_getall('tiny_wmall_goods_category', array('uniacid' => $_W['uniacid']), array('id', 'title'), 'id');
			$ids_str = implode(',', array_keys($cart));
			$goods_info = pdo_fetchall('SELECT id,sid,cid,title,number,price,total,total_warning,total_update_type,print_label,svip_status,svip_price FROM ' . tablename('tiny_wmall_goods') ." WHERE uniacid = :aid AND sid = :sid AND id IN ($ids_str)", array(':aid' => $_W['uniacid'], ':sid' => $sid), 'id');
			foreach($cart as $value) {
				...
				foreach($options as $option_id => $item) {
					...
					if($item['num'] > 0) {
						$stat['oid'] = $order['id'];
						$stat['uniacid'] = $order['uniacid'];
						$stat['agentid'] = $order['agentid'];
						$stat['sid'] = $sid;
						$stat['uid'] = $order['uid'];
						$stat['goods_id'] = $goods_id;
						$stat['goods_cid'] = $goods['cid'];
