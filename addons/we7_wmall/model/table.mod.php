...
function table_order_jiacai($sid, $order_id) {
	...
	if(empty($order['pay_type']) && empty($order['is_pay'])) {
		return error(-1,'订单未支付，不能加菜');
	}
	...
	$update = array(
		'num' => $order['num'] + $cart['num'],
		'total_fee' => $order['total_fee'] + $cart['price'],
		'final_fee' => $order['final_fee'] + $cart['price'],
		'store_final_fee' => $order['store_final_fee'] + $cart['price'],
	);
