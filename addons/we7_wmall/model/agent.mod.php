...
function agent_update_account($id, $fee, $trade_type, $extra, $remark = '', $order_type = 'takeout') {
	...
	if(!empty($id)) {
		...
		$agent_new = pdo_get('tiny_wmall_agent', array('uniacid' => $_W['uniacid'], 'id' => $log['agentid']), array('id', 'amount'));
		$text = "代理id:{$agent['id']},变动前金额:{$agent['amount']},金额变动:{$fee},变动后金额:{$now_amount},实际变动后金额:{$agent_new['amount']}";
		if($status === false) {
			slog('agentaccount', '代理账户变动失败', array(), $text);
		}
	}
	if($trade_type == 3) {
		mlog(5002, $id, "{$remark}, {$text}");
	}
	return true;
}
