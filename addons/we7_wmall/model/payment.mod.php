...
function alipay_build($params, $alipay = array()) {
	...
	if($trade_type == 'APP') {
		...
		$biz_content = array(
			...
			'out_trade_no' => $tid,
			'total_amount' => $params['global_fee'],
			'product_code' => 'QUICK_MSECURITY_PAY',
			'goods_type' => 1,
		);
		$set['body'] = "{$_W['uniacid']}:{$alipay['channel']}";
		...
		$string = rtrim($string, '&');
		if($alipay['channel'] == 'ttapp') {
			$orderInfo = array(
				'out_order_no' => $tid,
				'uid' => $_W['member']['uid'],
				'total_amount' => $params['global_fee'],
				'subject' => $params['title'],
				'body' => "{$_W['uniacid']}:{$alipay['channel']}",
				'trade_time' => $params['addtime'], //下单时间
				'notify_url' => $set['notify_url'],
				'alipay_url' => $string
			);
			mload()->classs('ttapp');
			$ttappClass = new Ttapp();
			$orderInfo = $ttappClass->buildOrderIfo($orderInfo);
			return array('orderInfo' => $orderInfo, 'order_sn' => $params['tid']);
		}
...
function alipay_pc_build($params, $alipay = array()) {
	...
	$set['payment_type'] = 1;
	$set['body'] = "{$_W['uniacid']}:wap:{$alipay['plugin']}";
	$prepares = array();
	foreach($set as $key => $value) {
		if($key != 'sign' && $key != 'sign_type') {
			$prepares[] = "{$key}={$value}";
...
/*	ini_set('date.timezone', 'Asia/Shanghai');
	...
	if($wechat['version'] == 1) {
		...
		if($package['trade_type'] == 'MWEB')  {
			$package['scene_info'] = json_encode(array(
				'h5_info' => array(
					'type' => 'Wap',
					'wap_url' => $_W['siteroot'],
					'wap_name' => $_W['we7_wmall']['config']['mall']['title'],
				),
			));
		}
		...
		if($package['trade_type'] == 'MWEB')  {
			...
			if($wechat['channel'] == 'ttapp') {
				$orderInfo = array(
					'out_order_no' => $params['uniontid'],
					'uid' => $_W['member']['uid'],
					'total_amount' => $params['global_fee'],
					'subject' => $params['title'],
					'body' => $_W['we7_wmall']['config']['mall']['title'] . '-' . $params['title'],
					'trade_time' => $params['addtime'], //下单时间
					'notify_url' => $package['notify_url'],
					'wx_url' => $wOpt['mweb_url'],
					'wx_type' => 'MWEB'
				);
				mload()->classs('ttapp');
				$ttappClass = new Ttapp();
				$orderInfo = $ttappClass->buildOrderIfo($orderInfo);
				$wOpt = array('orderInfo' => $orderInfo, 'order_sn' => $params['tid']);
			}
		}
		return $wOpt;
	}
...
function yimafu_build($params, $yimafu) {
	...
	$package = array(
		...
		'customerId' => $yimafu['mchid'],
		'notifyUrl' => base64_encode(urlencode(WE7_WMALL_URL . "payment/yimafu/notify.php?uniacid={$_W['uniacid']}")),
		'successUrl' => base64_encode(urlencode($params['url_detail'])),
		'uid' => 'we7_wmall',
		'goodsName' => cutstr($params['title'], 26),
		'remark' => "{$_W['uniacid']}:wap",
	);
	ksort($package);
	$str = '';
...
function pc_pay_prep($args) {
	...
	$tables_router = array(
		'plugincenter' => array(
			'table' => 'tiny_wmall_plugincenter_order',
			'order_sn' => 'order_sn'
		)
	);
	...
	$routers = array(
		'plugincenter' => array(
			'title' => "购买插件-{$record['order_sn']}",
			'url_detail' => iurl('plugin/plugincenter/buy', array(), true),
