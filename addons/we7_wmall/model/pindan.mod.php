...
function pindan_data_init($sid, $pindan_id, $ignore_bargain= false, $extra=array()) {
	...
	foreach($groups as &$cart) {
			...
			if(!$goods['is_options']) {
				...
				foreach($v as $key => $val) {
					...
					$cart_item = array(
						...
						'num' => $num,
						'price' => $goods['price'],
						'discount_price' => $goods['price'],
						'discount_num' => 0,
						'price_num' => $num,
						'total_price' => round($goods['price'] * $num, 2),
						'total_discount_price' => round($goods['price'] * $num, 2),
						'bargain_id' => 0
					);
		...
		if($extra['is_submit']) {
			...
			//$pindan_data['original_data'][$cart['uid']] = $cart_goods_original;
			//$pindan_data['all_goods'][] = $pindan_goods;
		}
