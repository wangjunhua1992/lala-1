...
function activity_cron() {
	...
	pdo_query("update " . tablename('tiny_wmall_store_activity') . " set status = 0 where uniacid = :uniacid and endtime < :time and type in({$type_basics})", array(':uniacid' => $_W['uniacid'], ':time' => TIMESTAMP));
...
function activity_del($sid, $type = '') {
	...
	if(empty($type)) {
		...
		if($_W['role'] == 'clerker' && !empty($_W['we7_wmall']['config']['store']['activity']['perm'][$type]) && $_W['we7_wmall']['config']['store']['activity']['perm'][$type]['cancel_status'] != 1) {
