...
function coupon_before_timeout_notice() {
	...
	if(!empty($coupons)) {
		...
		foreach($data as $item) {
			...
			$cols1 = pdo_query("update " . tablename('tiny_wmall_activity_coupon_record') . " set is_notice = 1 where uniacid = :uniacid and id in ({$recordids})", array(':uniacid' => $_W['uniacid']));
			$cols2 = pdo_query("update " . tablename('tiny_wmall_activity_coupon_record') . " set noticetime = {$timeout} where uniacid = :uniacid and status = 1 and uid = :uid", array(':uniacid' => $_W['uniacid'], ':uid' => $item['uid']));
			if(!empty($cols1) && !empty($cols2)) {
				$params = array(
					'first' => "{$item['nickname']}，您的账户下有{$item['num']}张优惠券即将过期，总价值{$item['discount']}{$_W['Lang']['dollarSignCn']}，记得使用哦~",
					'keyword1' => "账户优惠券",
...
function coupon_give_to_friend($couponOrid, $toUid) {
	...
	if($coupon['give_status'] > 0) {//1可赠送，2,不可赠送,3赠送中， 4赠送成功
...
function coupon_give_available_check($toUid = 0) {
	...
	if($config_coupon['give_num'] > 0) {
		...
		if($give_num >= $config_coupon['give_num']) {
			return error(-1, '今日赠送代金券已达上限');
		}
	}
	if($toUid > 0) {
		if($config_coupon['accept_num'] > 0) {
			...
			if($receive_num >= $config_coupon['accept_num']) {
