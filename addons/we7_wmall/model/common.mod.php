...
function set_system_config($key, $value) {
	global $_W;
	$ignoreagent = false;
	if(in_array($key, array('payment_rsa'))) {
		$ignoreagent = true;
	}
	$sysset = get_system_config('', -1, $ignoreagent);
	$keys = explode('.', $key);
	$counts = count($keys);
	if($counts == 1) {
		$sysset[$keys[0]] = $value;
...
if(!function_exists('get_system_config')) {
	function get_system_config($key = '', $uniacid = -1, $ignoreagent = false) {
		global $_W;
		if($uniacid == -1) {
		...
		$_W['is_agentconfig'] = 0;
		if($_W['agentid'] > 0 && empty($ignoreagent)) {
...
function get_available_payment($order_type = '', $sid = 0, $all = false, $orderType = 1) {
	...
	if(empty($order_type)) {
		...
		if(is_wxapp()) {
			...
			$payment = $payment['ttapp'];
			$index = array_search('alipay', $payment);
			if($index !== false) {
				unset($payment[$index]);
			}
...
function get_agent_perm($key = '', $agentid = 0) {
	...
	$agent['plugins'] = array(
		'errander', 'bargain', 'gohome', 'diypage', 'zhunshibao', 'superRedpacket'
	);
	if(!empty($key)) {
		return $agent[$key];
	}
...
function get_all_perms($justkey = false) {
	$all_perms = array(
		...
		'merchantaccount' => array(
			'title' => '商户账户',
			'perms' => array(
				'merchant.account.index' => '账户列表',
				'merchant.account.turncate' => '账户清零',
				'merchant.account.changes' => '账户变动',
				'merchant.account.yucunjin' => '预存金变动',
				'merchant.account.set' => '账户设置',
				'merchant.account.deposit' => '保证金设置',
			)
		),
		'service' => array(
			'title' => '售后',
		...
		'deliveryer' => array(
			'title' => '配送员',
			'perms' => array(
				'deliveryer.plateform.list' => '配送员',
				'deliveryer.policy' => '配送策略',
				'deliveryer.location' => '配送员位置日志',
				'deliveryer.getcash' => '提现申请',
				'deliveryer.current' => '账户明细',
				'deliveryer.plateform.turncate' => '账户清零',
				'deliveryer.plateform.changes' => '账户变动',
	...
	if(check_plugin_perm('agent')) {
		$all_perms['agent'] = array(
			'title' => '区域代理',
			'perms' => array(
				'agent.agent.list' => '代理列表',
				'agent.agent.set' => '账户设置',
				'agent.agent.changes' => '账户变动',
	...
	if(check_plugin_perm('vipRedpacket')) {
		$all_perms['vipRedpacket'] = array(
			'title' => '会员红包',
			'perms' => array(
				'vipRedpacket.meal' => '活动列表',
				'vipRedpacket.order' => '购买记录',
				'vipRedpacket.urls' => '入口链接'
			)
		);
	}
	if($justkey) {
...
function mlog_types($role = '', $value = 0) {
	...
	$type_all = array(
		'manager' => array(
			...
			'4003' => array('key' => 4003, 'type' => '后台变动配送员账户', 'content'=>'后台变动配送员账户(记录id:%s), 备注：%s'),
